...
def render_candlestick_card(row: Row, spec: DangerSpec, ohlc: OHLC, *,
    ...
    with st.container(border=True):
        ...
        # ── 2026-08-28:腳註原本寫「該欄**目前恆為 0**」,那句話當天就已經是假的
        #    (2026-08-26 真量 4,026,600 回來,streak 中斷)。畫面上掛一句假話
        #    正是 §1「錯誤的數字比沒有數字更危險」的直接命中,故就地更正。
        #    ⚠️ 新句刻意寫成**條件式**而不是「恆為 0」或「已恢復」——
        #    後兩者都是在陳述一個**會變的當下狀態**,寫死就保證會再過期一次
        #    (「恆為 0」活了不到一個月就變成假話,不要再種一個)。
        #    「缺值時為 0，與『真的沒成交』無法區分」陳述的是這個欄位的
        #    **永久性質**:0 在這一欄是有歧義的,無論來源當下有沒有掉量都成立,
        #    而且它同時就是本卡不畫量的理由本身。
        st.caption(f"{threshold_caption(spec)}　·　不畫成交量（該欄缺值時為 0，與「真的沒成交」無法區分）"
                   + (f"　·　{series_note}" if series_note else ""))
