...
def render_data_health_raw():
    ...
    with _bn1:
        st.info(
            ...
            '🔴 抓不到／過舊：兩種都紅 —— ①**真失敗**（未取得，API/proxy/網路問題）；'
            '②**落後 N 期**（有抓到但資料已過期，多半是上游還沒發布或 US-IP 被擋，'
            '**不是程式當掉**）。要分辨看「來源」與 log。'
        )
    with _bn2:
    ...
    if not _anom_rows:
        ...
        st.caption(
            '💡 **燈號語意**：🔴 抓不到／過舊（①真失敗＝API/proxy/網路；②落後 N 期＝抓到但過期，'
            '多半上游未發或 US-IP 被擋，**非程式當掉**）｜🟡 時效延遲或待補抓（仍可參考）；'
