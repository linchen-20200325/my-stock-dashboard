...
def _mj_load_holdings_callback() -> None:
    """帶入 Google Sheet 持股組合代碼 → 填入批次輸入框(on_click callback,rerun 前執行)。

    v19.160 user 要求:除「貼清單」外多一鍵帶入自己的持股組合。
    §8.2.A EX-PASSTHRU-1:L5 lazy import L1 gsheet_portfolio(pass-through、無 L3 業務值)。
    graceful:未登入 Google / 未設組合 → 只回 warn 訊息,不炸。
    """
    try:
        from src.data.portfolio import gsheet_portfolio as _gsp  # EX-PASSTHRU-1
        names = _gsp.list_portfolios()
        if not names:
            st.session_state["_mj_holdings_msg"] = (
        ...
        for _nm in names:
            for _r in (_gsp.load_portfolio(_nm) or []):
                ...
                if _t and _t not in tickers:
                    tickers.append(_t)
        if not tickers:
            st.session_state["_mj_holdings_msg"] = ("warn", "雲端持股組合是空的。")
            return
        capped = tickers[:10]  # 批次上限 10(parse_codes limit=10 對齊)
        st.session_state["_mj_batch_input"] = " ".join(capped)
        _tail = f"(共 {len(tickers)} 檔,取前 10)" if len(tickers) > 10 else f"(共 {len(tickers)} 檔)"
        st.session_state["_mj_holdings_msg"] = ("ok", f"已帶入持股 {_tail}:{' '.join(capped)}")
...
def render_mj_health_diff_tab(*, seed_codes: str | None = None) -> None:
    ...
    with st.container(border=True):
        ...
        # v19.160:除貼清單外,一鍵帶入自己的 Google Sheet 持股組合
        st.button("🔗 帶入我的持股（Google Sheet 組合）", key="_mj_load_holdings",
                  on_click=_mj_load_holdings_callback)
        _hmsg = st.session_state.pop("_mj_holdings_msg", None)
        if _hmsg:
