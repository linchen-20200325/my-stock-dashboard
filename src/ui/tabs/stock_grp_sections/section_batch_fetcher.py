...
from src.compute.strategy import (
    ...
    calc_vcp,
    calc_volume_ratio,
    summarize_caisen,  # v19.164:蔡森批次化(df4 已在手,零額外抓取)
)
...
def run_batch_fetch(stock_list: list[str]) -> None:
    ...
    for i4, sid4 in enumerate(stock_list):
        ...
        try:
            ...
            # ── 蔡森型態目標價(批次,v19.164)──────────────────────────
            # df4 已在手(上方 ThreadPool 抓),蔡森 L2 為純函式 → 零額外抓取、純 CPU。
            ...
            try:
                ...
                print(f'[section_batch_fetcher 蔡森] {sid4} '
            ...
            results_t3.append({
                ...
                '_caisen':     _caisen4,   # v19.164:蔡森批次摘要(供總表蔡森欄 + 下鑽 seed)
            })
