...
均價多少」。
⚠️ **這句全稱句是別組（AUD-5 稽核）窮舉 `src/services/` 全部 public 函式後的結論，
本檔作者沒有自己重跑那次窮舉**（§-2 規則 6 據實標明）。本檔的存在**不依賴**它成立：
就算真的漏了一支同義的舊介面，那也是「重複取數實作」要合併的問題（v3 §01-2），
不會讓本檔的唯讀契約或呼叫端的四態判定變錯。
