"""v18.172 短線風險雷達 (Risk-Off Velocity Radar) — 9 燈快速訊號（v18.320 原 10 燈）

設計動機（v18.172 epic，鏡像 fund v19.20）：
拐點偵測中心 5 大訊號全是「月～季級慢速 macro inflection」(PMI/Sahm/CFNAI/yield curve/HY 90D max)，
對 6/5/2026 那種「1-day 急殺」事件結構性盲眼。本模組補 1～5 day 動量／情緒／位階訊號，
與拐點偵測中心互補（慢 vs 快）。

9 燈清單（v18.320：原第 9 燈 put_call_ratio 四源全死下線，詳見 _RADAR_KEYS 上方註解）：
  ...
  9. asia_overnight    — Nikkei + HSI 亞洲夜盤平均跌幅
...
# v18.320 Put/Call 燈整盞下線（user 2026-06-27 實測四源全死）：
#   Yahoo ^CPC/^CPCE → 404；CBOE CPC/CPCE_History.csv → S3 AccessDenied；
#   stooq ^cpc/^cpce → No data。CBOE 多年前停止公開 equity P/C CSV，已無免費源。
#   → 從 9 燈雷達移除（原 10 燈），不再每次 load 打 4 個死 endpoint。
_RADAR_KEYS = (
    "vix_level",
    "vix_term_struct",
    "hy_oas_delta",
...
def _resolve_vix3m() -> tuple[pd.Series, str]:
    """VIX3M 多源 chain（v18.320 精簡 — user 2026-06-27 實測確認死活）：
        Yahoo ^VIX3M（活，20.13 有報價）→ CBOE VIX3M_History.csv（活，下載 210KB）。
    已移除 Yahoo ^VXV（回 0.0000 已停更）+ stooq ^vix3m/^vxv（回 'No data'），
    避免無謂的 4 次 timeout。"""
    s = fetch_yf_close("^VIX3M", range_="6mo")
    if not s.empty and len(s) >= 2:
        return s, "Yahoo ^VIX3M"
...
# v18.320 _fetch_stooq_csv / _resolve_put_call 已刪除：
#   - stooq 對 ^vix3m/^vxv/^cpc/^cpce 全回 'No data'（user 實測 error.txt 13 bytes）。
#   - Put/Call 四源全死（見 _RADAR_KEYS 上方註解），整盞燈下線，無 caller。
...
# v18.320 — 原「9. CBOE Put/Call Ratio」整盞下線：四源全死、已無免費源
#   （詳見檔頭 _RADAR_KEYS 上方註解）。_signal_put_call_ratio / _resolve_put_call 已刪。


# ── 9. 亞洲夜盤（Nikkei + HSI 平均單日跌幅）──────────────────────
...
def detect_risk_radar(fred_api_key: str) -> dict[str, dict]:
    """9-light 短線風險雷達（v18.320：原 10 燈，Put/Call 四源全死下線）。

    回傳 dict keyed by signal id (9 keys)：
    ...
    單一訊號抓取失敗 → 該 key 回傳 _empty() ⬜，其餘 8 燈不受影響。
    """
    return {
...
def summarize_radar(radar: dict[str, dict]) -> dict:
    """彙總 9 燈狀態 → 摘要 dict（v18.320：原 10 燈，Put/Call 下線）。
...
def synthesize_dual_verdict(
    ...
    *,
    ...
    v19.170 持股數字下架（SSOT 合規）
    ----------------------------------
    本函式原本在各分支硬編碼「現金 30%+ / 倉位降至 50-60% / 現金 25-30% /
    現金 35%+ / 倉位 60-70%」，經 `helpers.synthesize_dual_verdict` → 「🤝 雙速合議」
    卡直接印給使用者，與 🎚️ 建議持股油門（`allocation_service.get_allocation()`，
    全站唯一持股 SSOT）互相打架。
