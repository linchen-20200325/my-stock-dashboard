...
#
# ── 為什麼是 15,不是 12 或 18(實測校準,不是拍腦袋的 75%)──────────────
# 對同一份 parquet 做 `(volume == 0).rolling(20).sum()`,**排除 2026-07~08
# 那段 33 日 streak** 之後,全歷史(2007~2026-06)每個 20 日視窗的假 0 筆數
# **最大值 = 5**(2019-12-04/05/09/10/18 五天擠在同一個視窗)
# → 歷史上最壞的零星缺值情況,有效樣本**恰好是 15**。
#
#   · 設 **18**(最多容忍 2 筆)→ 2019-12 那個視窗會被判定「資料不足」而關掉
#     濾網,但當時明明有 15 筆真實觀測可用 —— 把可算的日子誤殺成不可算。
#   · 設 **12**(容忍 8 筆)→ 等於宣稱「12 天的平均」可以叫做「月均量」,
#     而畫面印的是「X.Yx **均量**」;分母的視窗與文案講的視窗對不上,
#     本身就是一種不誠實(且缺值若集中在高量日,剩下的 12 筆是有偏樣本)。
#   · 設 **15** → 涵蓋歷史上出現過的所有零星缺值型態,而**比歷史最壞情況
#     還糟**的(例如 2026 那種整段缺)一律誠實回報未評估。
#
# ⚠️ **為什麼可以排除那段 streak(不是循環論證)**:本門檻的功能定義就是
# 「區分**零星缺值**(仍可算)vs **系統性斷線**(不可算)」。拿系統性斷線去校準
# 「零星」的容忍度是**類別錯誤** —— 那段 33 日 streak 正是本門檻要擋下來的東西,
# 不是用來決定門檻高低的樣本。
#
# ⚠️ **這道門檻管的是「筆數」,不管「誤差量級」—— 殘留風險在此據實揭露**:
# 15 筆有效樣本只保證「樣本夠不夠多」,**不保證剩下那 15 筆的平均接近真實 20 日均量**。
# 若掉的 5 天剛好是**量最大**的 5 天(最壞相關情境),均量會被低估、量比被高估。
# 實測全部 4,777 個乾淨視窗模擬掉最大 5 天:
#     膨脹倍數 中位數 1.073 / p99 1.182 / 最大 1.261
#     足以把原本 ≤1.3 的量比推過 1.3(＝假瘋牛):約 325~331 / 4777 = **6~7%**
# 也就是說:**本門檻把「整段缺值」那種假瘋牛降到 0,但零星缺值仍留約 6~7% 的上界風險。**
# ⚠️ 上面否決 12 的理由(「缺值若集中在高量日 → 有偏樣本」)**同樣適用於 15**,
# 只是程度較輕 —— 這裡把同一把尺套回自己,不做選擇性適用。
#
# ⚠️ **證據強度據實標明**:這個常數實質上由 20 年裡的**一個事件**(2019-12)單點決定,
# **n = 1**。兩個方向的代價都不高(誤殺 = 多說一次「資料缺失」;放行 = 上述 6~7%),
# 所以可接受;但**證據強度比上面這 20 幾行註解讀起來的樣子弱**,不要當成統計結論。
# ⚠️ 15 是**依既有歷史校準**出來的,不是從既有 SSOT 推導的;
# 上游若哪天改變掉量的行為型態,這個數字要重新量,不要沿用本行。
