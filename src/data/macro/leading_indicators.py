...
#
# ── B4-a 覆審:為什麼**維持** 3 日、不放寬 ──────────────────────────────
# 稽核提問:放寬上限會讓更舊的 pickle 被讀進來(但至少帶 is_stale 旗標),
# 不放寬則「9 天凍結」偵測不到 —— 這個取捨怎麼選?
# 結論:維持 3 日,理由三點。
#  ① 放寬**解不了**該事故。超齡回 None 之後,tab_macro 的 else 分支會沿用
#     session_state 既有的 li_latest,而那份 df 可以任意老(它跟 pickle 年齡
#     無關)。也就是說「有一份沒旗標的舊資料被顯示」這件事,把上限調到 30 天
#     照樣會發生;真正的洞在**沿用路徑沒有標記**,已於 tab_macro 補上
#     `li_retain_meta`(見該處註解)。
#  ② 凍結偵測不依賴這個常數。`shared.data_freshness.detect_frozen_columns`
#     量的是**值的一階差分**,不管資料是 pickle 還是即時抓的,只要連續 N 期
#     沒動就會被點名(B4-a 已在 🔎 資料診斷頁接上)。放寬上限只是讓更多舊資料
#     流進系統,並不會讓偵測器更靈敏。
#  ③ §1 的方向是「寧可炸掉,不可造假」。先行指標是**當日決策**用表
#     (外資留倉 / 未平倉 / 三大法人),超過 3 個交易日的快照拿來當今日盤勢
#     判讀,帶不帶旗標都已無參考價值;顯示「抓不到」比顯示「4 天前的數字 +
#     一個小徽章」更難被誤用。
# 若未來需要放寬(例如春節長假 9 天封關),正確做法是改成「以交易日計算年齡」
# 而非單純把分鐘數調大 —— 目前 3 個日曆日在連假期間確實偏嚴,但偏嚴的後果是
# 「顯示缺資料」(安全),不是「顯示假資料」(危險),故不在本輪動。
