...
EDU_GUIDE: dict[str, dict[str, Any]] = {
    ...
    'BWIBBU_d': {
        ...
        'how_to_read': [
            ...
            ('PB < 1 ／ PE > 30',
             # 判定式的實作位置(shared/thresholds.classify_yield_zone)屬開發者資訊,
             # 留在本註解;下面那句是渲染給使用者看的,不寫模組路徑。
             '⚠️ 通則參考 —— 本系統沒有用 PB／PE 絕對值做自動判定，'
             '個股估值一律走上方 357 殖利率四段'),
        ],
    ...
    'MI_MARGN': {
        ...
        # 技術備忘(不渲染給使用者):v19.170 稽核實測 5,148 億,已同時穿透兩線;
        # 相對分位方案見 shared/relative_thresholds(尚未接線,屬行為變更需另案)。
        # 下面渲染出去的版本只講「為什麼會一直紅」與「該改看什麼」。
        'historical_anchor': '2007 高峰 4400 億（2008 崩盤前）｜2022/4 高點 2900 億'
                             '｜⚠️ 這兩條是絕對金額門檻，沒有隨市場總市值成長調整 —— '
                             '近年餘額已長期高於 §§MARGIN_WARN_YI§§ 與 §§MARGIN_OVERHEAT_YI§§ '
                             '兩線，燈號會持續偏紅；判讀時請看「相對自身近年區間的變化方向」，'
                             '而非只看有沒有超過門檻',
    ...
    '前五大留倉': {
        ...
        # 技術備忘:v19.170 稽核實測連續 9 個交易日數值完全不變(資料源凍結)。
        'historical_anchor': '⚠️ 本欄沒有可查證的歷史「健康區間」，本手冊不編一個。'
                             '可驗證的資料特性：本欄靠期交所（TAIFEX）網頁抓取，'
                             'FinMind 免費版沒有這份資料，實務上曾出現連續多個交易日'
                             '數值完全不動（資料停更）—— '
    ...
    '前十大留倉': {
        ...
        'how_to_read': [
            ('（本欄無自動判定式）',
             '⚪ 只呈現數字，不亮燈 —— 本欄沒有任何自動判定式。'
             '⚠️ 別把外資期貨的門檻（黃 §§FUT_YELLOW_LOTS§§ 口／'
             '紅 §§FUT_RED_LOTS§§ 口）套過來，兩者統計母體不同'),
            ('改看「前五大留倉」',
        ...
        # 技術備忘:v19.170 稽核實測連續 9 個交易日凍結不動。
        'historical_anchor': '⚠️ 本欄沒有可查證的歷史「健康區間」，本手冊不編一個。'
                             '可驗證的資料特性：資料來自期交所（TAIFEX）網頁抓取'
                             '（FinMind 免費版沒有這份資料），實務上曾出現連續多個交易日'
                             '凍結不動 —— 請先看 🔎 資料診斷頁的凍結偵測',
    ...
    'XTEXVA01TWM664S': {
        # 技術備忘:v18.142 由 IMF IFS(VALEXPTWM052N)改採 OECD MEI,月延遲短 10 個月。
        'meaning': '台灣每月出口金額年增率，全球景氣與台廠訂單能見度的核心指標'
                   '（台灣 GDP 約 60% 來自出口）。'
                   '本系統採用 OECD 的統計版本，比另一個常見的 IMF 版本'
                   '早約 10 個月拿到最新月份資料。',
        'how_to_read': [
