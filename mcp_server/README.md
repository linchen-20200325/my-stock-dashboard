...
`factors` 任選：`pe_low`(低估值)、`eps_high`(高EPS)、`shortage`(缺貨動能)、`rs_leader`(抗跌RS)、`trend`(跨季轉強)；留空＝全 5 因子。
回傳含 `as_of`(UTC 抓取時間)、`picks`(代碼/名稱/綜合分/各因子分)。**季快照未就緒 → `ok:false` + 原因，不編造清單**（§1 fail-loud）。
...
- **唯讀**：首波只查，不下單、不寫入。
- **資料前提**：`screen_stocks` 需季財報快照（`data_cache/fundamentals/`，由 `update_fundamentals` cron 產）+ 網路可達 TWSE；缺任一 → 回 `ok:false` 說明，不炸。
- **第二波候選**（未做，等有需求）：個股體質（`financial_health_engine`）、總經紅綠燈（`macro_state_locker`，需補無頭編排）、前進式驗證對帳（`forward_test_service`）。
