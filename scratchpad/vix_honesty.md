...
（全檔 grep `green_below` / `全站綠線` 皆 0 命中）。**出處已查證找到**：`stock_truth_spec.md`
—— 它**不在 repo 內**，位於本次工作的暫存工作區；repo-scoped
`find . -name "stock_truth_spec.md" -not -path "./.git/*"` → 0 命中**是正確的**，
那個檔本來就不在版控裡（全機器 `find / -name "stock_truth_spec.md"` 才命中）。
⚠️ **但該規格原文寫的是「讀憲法的人會以為『全站綠線是 18』」，並沒有說「§3.2 把 18 列在表上」**
（實測：該檔全檔未出現 `§3.2`）——**「列在表上」是下游轉述時才加上去的。**
唯一的真實關聯是：§3.2 那一列的 evidence 欄指向 `macro_core.py:215`，**順著指標過去**
才看得到 `green_below=18`；**表上本身沒有 18**，所以「列在範圍表上」仍然是錯的敘述。
**要轉給憲法組的真正內容**是這個事實本身：**`green_below=18` 是死參數、全站綠線並不是 18**——
而不是「憲法列錯了」。本次未動 CLAUDE.md 一個字。
⚠️ 這一句前後更正了**三輪**：
① 原始假宣稱（本組沒查證就寫）→
② 總管指示的更正**又夾帶一個未查證的出處歸屬** →
③ 該歸屬其實**查得到**，只是不在 repo 裡；而且**連原始出處都沒說「列在表上」**，
那三個字是轉述時長出來的。
**三輪都是同一種病 —— 用轉述當事實，而且每一輪都以為自己已經修好了。**
