...
| `financial_health_thresholds.MJ_*`（19 個 老師 門檻） | 現金 25/10 / DSO 15/90 / 100-100-10 / 負債 40/60/70 / 流動·速動 300/150 / 毛利 40 / 安全邊際 60 / 淨利 10 / ROE 15 / 杜邦槓桿 65 / 盈餘品質 100 等 | shared/financial_health_thresholds.py v18.323 | ✅ SSOT（financial_health_engine 6 個 `_no_ai_*` code 端引入；prompt 端由 golden test 釘一致；含 3 漂移修正,詳見 SPEC §13） |
...
| **L5 UI Tabs** | Streamlit Tab 級組裝 | `tab_macro.py`、`tab_stock.py`、`tab_stock_grp.py`、`tab_stock_picker.py`、`caisen_targets_ui.py`(老師目標價,`render_caisen_for_ticker` 內嵌 🔬 個股 + 🏆 個股組合;v19.164 組合改**批次表 + 下鑽共用批次 df**,無獨立分頁)、`etf_dashboard.py`、`etf_tab_*.py`(**~~tab_mj_health_diff.py~~ v19.164 退役真刪**:MJ「找體質差→變好」轉機能力已合併進 🏆 個股組合「📊 老師 趨勢×轉機」區塊 — `compute_one_stock_trend` 用同一份季快照附帶算 `diff_verdict`,零額外抓取、去第二輸入框 + 去重複第二張表) |
