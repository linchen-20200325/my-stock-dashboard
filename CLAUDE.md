...
- §8.2.A 例外清單：EX-L0-1 / EX-CACHE-1 / EX-PASSTHRU-1(EX-AI-1 + EX-RENDER-1 已退役)
...
| **EX-PASSTHRU-1** | UI 直呼以下 L1 fetcher(無對應 L3 業務 wrapper)— **已收齊 13 處**(P5-B2 v18.396 補 6 處):<br>**Module-level import(7 處)**:<br>- `app.py:62` `from src.data.core import StockDataLoader, _LOADER_VERSION`<br>- `src/ui/etf/etf_dashboard.py:16` `from src.data.etf import ...`<br>- `src/ui/etf/etf_tab_grp_compare.py:25` `from src.data.etf import fetch_etf_price, fetch_etf_dividends, fetch_etf_info`<br>- `src/ui/tabs/tab_stock_grp.py:37` `from src.data.core import fetch_bps, fetch_industry_category`<br>- `src/ui/tabs/tab_stock.py:68` `from src.data.core import fetch_bps, fetch_industry_category`<br>- `src/ui/tabs/yield_screener.py:71` `from src.data.stock.dividend_fetcher import fetch_annual_dividends`<br>- `src/ui/tabs/macro/section_mid.py:17` `from src.data.macro import check_macro_alerts, fetch_macro_snapshot, render_macro_alerts`<br>**Lazy import(6 處,v18.396 P5-B2 補登)**:<br>- `src/ui/tabs/tab_edu.py:154` `from src.data.core import get_categories, get_by_category, ...`<br>- `src/ui/tabs/yield_screener.py:33,80` `from src.data.proxy import fetch_url / get_proxy_config`<br>- `src/ui/tabs/tab_mj_health_diff.py:85` `from src.data.core import fetch_financial_statements`<br>- `src/ui/tabs/macro/section_short.py:311` `from src.data.macro import fetch_twse_breadth`<br>- `src/ui/tabs/macro/section_state.py:140` `from src.data.macro import ...`<br>- `src/ui/tabs/tab_stock.py:245,2715` `from src.data.core import fetch_financial_statements / from src.data.stock import fetch_5_years_cash_flow`<br>(註 1:`src/ui/tabs/macro/section_chips.py:22` 也 `from src.data.macro import render_leading_table`,但 `render_leading_table` 是 render 函式而非 fetcher,實際合 L4 / 略豁)<br>(註 2:`src/ui/tabs/tab_macro.py:118,122` 在 render_tab_macro() 函式內 late import,屬 anti-circular pattern 而非真 L5→L1 違憲 / 略豁) | L5 UI Tab / L6 App 可直接 import L1 「pass-through 用 + 無 L3 業務值」的 fetcher | §8.2 規則「cache 才能集中」核心理由失效於本場景:L1 模組內已用 `@st.cache_data`(EX-CACHE-1)集中緩存,L3 wrapper 加一層只是 pure pass-through = §8.1 step 6「用不到的抽象」反例。Lazy import(6 處)額外的合理性:多在 button click handler 內,延遲 import 避免 module load 時跑全 dependency chain。**升級觸發條件**:若未來新增跨多 fetcher 統一 TTL、多源 fallback chain、或結果後處理 → 升級 L3 service。S-H5/S-H6 v18.244 + P2-EX v18.393 + P5-B2 v18.396 決策。 |
