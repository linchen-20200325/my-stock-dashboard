...
| **L5 UI Tabs** | Streamlit Tab 級組裝 | `tab_macro.py`、`tab_stock.py`、`tab_stock_grp.py`、`tab_stock_picker.py`、`caisen_targets_ui.py`(蔡森目標價,`render_caisen_for_ticker` 內嵌 🔬 個股 + 🏆 個股組合;v19.164 組合改**批次表 + 下鑽共用批次 df**,無獨立分頁)、`etf_dashboard.py`、`etf_tab_*.py`(**~~tab_mj_health_diff.py~~ v19.164 退役真刪**:MJ「找體質差→變好」轉機能力已合併進 🏆 個股組合「📊 MJ 趨勢×轉機」區塊 — `compute_one_stock_trend` 用同一份季快照附帶算 `diff_verdict`,零額外抓取、去第二輸入框 + 去重複第二張表) |
