...
| **EX-PASSTHRU-1** | UI 直呼以下 L1 fetcher(無對應 L3 業務 wrapper)— **已收齊 7 處**(P2-EX v18.393 補登錄):<br>- `app.py:62` `from src.data.core import StockDataLoader, _LOADER_VERSION`<br>- `src/ui/etf/etf_dashboard.py:16` `from src.data.etf import ...`<br>- `src/ui/etf/etf_tab_grp_compare.py:25` `from src.data.etf import fetch_etf_price, fetch_etf_dividends, fetch_etf_info`<br>- `src/ui/tabs/tab_stock_grp.py:37` `from src.data.core import fetch_bps, fetch_industry_category`<br>- `src/ui/tabs/tab_stock.py:68` `from src.data.core import fetch_bps, fetch_industry_category`<br>- `src/ui/tabs/yield_screener.py:71` `from src.data.stock.dividend_fetcher import fetch_annual_dividends`<br>- `src/ui/tabs/macro/section_mid.py:17` `from src.data.macro import check_macro_alerts, fetch_macro_snapshot, render_macro_alerts`<br>(註:`src/ui/tabs/macro/section_chips.py:22` 也 `from src.data.macro import render_leading_table`,但 `render_leading_table` 是 render 函式而非 fetcher,實際合 L4 / 略豁) | L5 UI Tab / L6 App 可直接 import L1 「pass-through 用 + 無 L3 業務值」的 fetcher | §8.2 規則「cache 才能集中」核心理由失效於本場景:L1 模組內已用 `@st.cache_data`(EX-CACHE-1)集中緩存,L3 wrapper 加一層只是 pure pass-through = §8.1 step 6「用不到的抽象」反例。Stock 端尚未建立完整 L3 service 層,DataLoader class 既有方法已含跨頻 cache 邏輯;**升級觸發條件**:若未來新增跨多 fetcher 統一 TTL、多源 fallback chain、或結果後處理 → 升級 L3 service。S-H5/S-H6 v18.244 + P2-EX v18.393 決策(類比 Fund EX-PASSTHRU-1)。 |
| **EX-RENDER-1**(新例外候選) | `src/ui/render/etf_render.py:11` `from src.data.etf import _fetch_news_for, _fetch_sector_returns` | L4 Render 直 import L1 Data fetcher | **嚴格違反**「L4 不得 import L1」分層硬規則。`_fetch_news_for` / `_fetch_sector_returns` 是純 read-side fetcher,被 etf_render 內部 Streamlit 元件直接 cache + render 消費。**緩解**:這 2 個 fn 已是 @st.cache_data,實質效果與 L3 wrapper 一致。**升級觸發條件**:若新增跨 fetcher 編排需求 → 抽 L3 service `etf_news_service`。v18.393 P2-EX 補登(原為未登錄軟例外)。 |
