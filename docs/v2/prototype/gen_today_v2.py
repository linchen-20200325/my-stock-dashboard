    ...
    return {
        ...
        "why.edu.legacy": [
            {"state": "unwired", "title": "④ 完整策略邏輯說明書（既有 📚 教學）",
             "value": None, "level": None, "badge_n": rb(state="unwired"),
             # 🔴 **2026-09-22 fact key 改名（決策者 客戶，執行 WC 組）—— ⛔ 不是漏刪。**
             #    舊 key 逐字帶一個被客戶點名禁上卡面的內部詞 ⇒ 改成**等義**的中性字。
             #    ⚠️ **只改本檔自己取的 key，⛔ 沒有動任何線框／規格的逐字** ——
             #       該 key 是本檔用來指「線框那一格上掛的記號」的自訂標籤，
             #       ⛔ 不是線框欄位名；線框那一格的**值**「★待拍板」一個字都沒動。
             #    ⚠️ 「📖 憑什麼」頁規格**自己沒有**內部詞禁令（本組 grep 實測 0 命中）
             #       ⇒ 本處的依據是**客戶 2026-09-22 的站級裁示**，⛔ 不是頁規格。
             #    舊 key 逐字與改因登記於本頁附錄 **W13**。
             "facts": (("實跑狀態", "unwired（規格 ③ 規則二 (b)）"),
                       ("線框標記", "★待拍板"))},
        ],
        # ── n2 逐盞門檻表（三張表 ＋ 兩段表下說明）────────────────────────────
...
WHY_SPEC_GAPS: tuple[tuple[str, str, str], ...] = (
    ...
    (
        "W13",
        "🆕 **`why.edu.legacy` 的一個 fact key 於 2026-09-22 改名** —— "
        "舊 key 逐字保存在本條（⛔ 不是刪掉）。",
        "（本條為**逐字引述**，含客戶點名禁上卡面的內部詞；"
        "依客戶 2026-09-22 裁示，附錄／缺口段可引述，但必須標明「引述」。）"
        "**舊 key 逐字**＝「線框旗標」，**新 key**＝「線框標記」；"
        "該 key 的**值**「★待拍板」一個字都沒動，**語意等義**。"
        "／／**改名的依據**＝ 客戶 2026-09-22 裁示的**站級**卡面禁詞"
        "（客戶列舉逐字含「旗標」二字）。"
        "🔴 **本頁規格自己⛔ 沒有內部詞禁令**（WC 組 grep 實測：`UI_PAGE_WHY.md` 對"
        "「不得出現／禁用／內部詞／旗標」**0 命中**）⇒ 本處⛔ 不得被引用為「頁規格要求的」。"
        "／／⚠️ **⛔ 不是動線框**：該 key 是本檔用來指「線框那一格上掛的記號」的**自訂標籤**，"
        "⛔ 不是線框的欄位名；線框檔本輪一個字都沒動。"
        "⚠️ 射程判讀（站級 vs 頁級）是 WC 組拍板、⛔ 非客戶逐字，已登記為缺口 **K6**。",
    ),
)
    ...
    return {
        ...
        "find.pe_two_kinds": [
            {"state": "missing", "title": "🔴 為什麼有些本益比是空的（PE 兩種「沒有」的分辨）",
             ...
             # 🔴 **2026-09-22 卡面改寫（決策者 客戶，執行 WC 組）—— ⛔ 不是漏刪。**
             #    舊的四條 fact **把禁令本身逐字引述到卡面上**（客戶裁示逐字 ＋ 規格 ④(d)
             #    的禁詞列舉），⇒ 禁令裡的每一個詞都被印上畫面，共 **7 次**命中。
             #    **舊寫法的理由仍然成立**：它讓讀者在卡上就查得到出處，是本檔的既有體例
             #    （那個體例與禁令的結構性衝突已登記為缺口 **K5**，WC 組⛔ 不裁決）。
             #    **被權衡掉的原因**：規格 ④(d) 的禁令射程逐字就是「**使用者看到的字面**」
             #    ⇒ 卡面沒有「標明引述就免責」這個豁免（那個豁免只給附錄／缺口段）。
             #    ⛔ **不是刪掉**：被拿掉的那兩條 fact 逐字保存在
             #    **本頁附錄 F18**（另：同一段客戶裁示逐字在 `FIND_LAYER_NOTE[4]`
             #    與缺口 **F8** 已各有一份，兩處皆已標明「引述」）。
             #    ⛔ **⛔ 不發明規格沒有的說法**：下面兩條的字面**取自規格 ④(d) 自己的擬稿**
             #    （`UI_PAGE_FIND.md:103`，該處自註為「本組延伸、⛔ 非客戶逐字」）。
             "facts": demo(
                 ("上畫面的字面", "一律標 ⚠︎ —，並就地說明「這一欄目前分不出兩種「沒有」」"),
                 ("就地說明（規格 ④(d) 擬稿）",
                  "本益比是空的時候，這一項目前分不出是「沒有這個數字」還是「公司不適用」，"
                  "所以一律標成 ⚠︎ —；在能分開之前，本頁不會替你猜是哪一種。"),
                 ("線框十態", "十格全部非 null，逐字都是「常駐」—— missing 那格另註「這一態正是它要解釋的」"),
                 ("技術原因寫在哪", "規格 ④(d) 明文禁止那一類字上畫面 ⇒ 原文逐字搬到本頁附錄 F18"))},
        ],
        # ── n5 葉2 板塊地圖（兩張圖各自判態）──────────────────────────────
        "find.map_cta": [
...
FIND_SPEC_GAPS: tuple[tuple[str, str, str], ...] = (
    ...
    (
        "F18",
        "🆕 **`find.pe_two_kinds` 的卡面於 2026-09-22 改寫** —— 被拿掉的兩條 fact "
        "**逐字保存在本條**（⛔ 不是刪掉）。",
        "（本條全段為**逐字引述**，含規格點名禁上卡面的內部詞；"
        "依客戶 2026-09-22 裁示，附錄／缺口段可引述，但必須標明「引述」。）"
        "**被拿掉的第 1 條**，key 逐字「客戶 2026-09-16 裁示逐字」、值逐字："
        "「UI 層先誠實顯示「本機分不出，需 L1 補旗標」。L1 修改列為獨立任務，"
        "等 UI 全部做完再評估解凍。不要假裝做得到」。"
        "**被拿掉的第 2 條**，key 逐字「⛔ 使用者看到的字面不得出現」、值逐字："
        "「L1／旗標／D-08 這類內部詞，也不得出現「虧損」」。"
        "／／**改寫的依據**＝ `UI_PAGE_FIND.md ④(d)`（`:105`）逐字：「⛔ 使用者看到的字面"
        "**不得出現**「L1」「旗標」「D-08」這類內部詞；⛔ **亦不得出現「虧損」**"
        "（線框 evidence 已定：系統手上沒有 EPS，說「虧損」＝ 替上游宣稱一件它沒說的事）」。"
        "⇒ 該禁令的射程逐字是「**使用者看到的字面**」，而客戶的「標明引述可免責」"
        "**只給附錄／缺口段** ⇒ 卡面必須改成不含那些字的中性敘述。"
        "／／⚠️ **⛔ 不是本組判某個字違規**：上列每一個詞都由規格或客戶**逐字點名**。"
        "⚠️ 其中「虧損」**前幾輪全部漏掉**（本輪由客戶點出，見缺口 K9 的出處更正）。"
        "⚠️ 這個體例衝突（卡面體例本來就是逐字引述）已登記為缺口 **K5**，WC 組⛔ 不裁決。",
    ),
)
...
HOLD_SPEC_GAPS: tuple[tuple[str, str, str], ...] = (
    ...
    (
        ...
        "／／📌 **2026-09-22 後續（本條以上全文一字未刪未改；下列是狀態更新，⛔ 不是改寫）**："
        "上文說的「另案」**已於同日執行**（客戶派工，WC 組），三處狀態更新 —— "
        "**(1) 總數 7 → 8**：`UI_PAGE_FIND.md ④(d)` 逐字另禁「虧損」二字（**引述**該條），"
        "而上文與前幾輪**全部漏掉它** ⇒ 實際是 **8 次、2 頁 2 卡**；"
        "**(2) 出處更正**：該禁詞條在 **④(d)**（`:105`），**⛔ 不是 ⑤**（⑤ 是反例自檢）—— "
        "上文寫「⑤」是**出處寫錯**，禁令內容本身一個字都沒有變（見缺口 **K9**）；"
        "**(3) 「⛔ 不自行擴表」已由客戶解除**：客戶派工單**逐字把 `D-08` 列進窮舉清單** "
        "⇒ `D-08` 與「虧損」已收進**頁級**表 `_BANNED_PAGE[find]`；"
        "同族的 `D-3` 客戶**仍未點名** ⇒ 比照舊處置**登記、⛔ 不擴表**（缺口 **K3**）。"
        "**(4) 「本輪不動另外兩頁」已執行完畢**：那 8 次已全部修掉，"
        "且逐張卡的禁詞守衛**自本日起裝在五頁全部**（原本只有 ⑭-h／⑮-h 兩頁）。"
        "⚠️ 上文的 (甲)(乙) 兩個口徑差異**仍然成立且仍然重要** —— "
        "「禁詞出現幾次」與「守衛回報幾條 hit 條目」是兩個數字，⛔ 不得混用。",
    ),
    (
        "H9",
...
#: 🆕 **第六組：卡面文案與禁詞射程**（2026-09-22 WC 組新增，前綴 `K`；
#: 編修前 grep `K1`~`K9` 於產生器與產物**均 0 命中**，⛔ 無編號碰撞）。
#: 🔴 **這一組⛔ 不屬於任何一頁** —— 它記的是「**禁詞這件事本身**」跨五頁的狀況，
#:    故與 `CHROME_SPEC_GAPS`（五頁共用 chrome）一起畫在共用附錄段。
#: ⚠️ ~~**本組九條全部是「登記、⛔ 不修」**~~ —— 客戶本輪只授權修 8 處字面 ＋ 補守衛；
#:    下列各條**皆未經客戶裁示**，⛔ 不得被讀成「WC 組已判定它們是違規」。
#: 📌 **2026-09-22 事實更正 —— 有意識的更正，⛔ 不是漏刪**（決策者 AI 總管，執行 WE 組）：
#:    畫掉的只有「**九條**」這個**會漂移的數字**（`CLAUDE.md §8.2.A.0 規則 4`）——
#:    WE 組本輪接續登記 `K10`~`K16` 共 7 條 ⇒ 該數字自本輪起不成立。
#:    ⚠️ **「全部是登記、⛔ 不修」這半句一個字都沒有被弱化，仍然完全有效** ——
#:    新增七條**同樣是登記、⛔ 不裁決**；本輪動到的只有**守衛**（禁詞表與結構 assert），
#:    **⛔ 沒有動任何一張卡的文案**（卡面 diff ＝ 0，由 WE 組實測）。
COPY_SPEC_GAPS: tuple[tuple[str, str, str], ...] = (
    (
        "K1",
        "🔴 卡面上有**內部函式名／模組名／enum 名**，**五頁全中** —— "
        "⛔ 本輪不修，因為修它必須**發明規格沒給的中文標籤**。",
        "WC 組實測（本組自己的正則，⛔ 不是轉述）：**30 次、散在 20 張卡、五頁全部有**。"
        "樣本：`card_value_text()`／`card_level_text()`／`st.button`／`st.stop()`／"
        "`CLAUDE.md`／`WIRING_DISCLOSURE`／`MISS_CONTRACT_DRIFT`／`UI_EMPTY`／分層代號。"
        "⚠️ **口徑差異，據實寫出**：前兩組（WA／WB）各自報「約 52 處」，本組數到 30 —— "
        "差在**正則集不同**（本組沒有把每一種命名形狀都寫進樣式），"
        "⛔ 不是誰算錯，**兩個數字都只能當單組量測**。"
        "⛔ **本輪不修的理由**：`S2-UI_SPEC.md` 的 **S-10** 確實寫「程式路徑、模組名、"
        "enum 名⛔ 不得進入可及名稱」，但規格**沒有給替代標籤** —— 例如 `today.holdings` "
        "的九個可顯示欄位，`UI_PAGE_TODAY.md` 只給了 `top1_pct` 一個中文名"
        "（逐字引述：「最大單一產業（檔數等權）」），**其餘八個一個都沒給**。"
        "自己編八個中文名 ＝ 發明規格沒有的東西（`CLAUDE.md §1`）⇒ **等客戶裁示**。",
    ),
    (
        "K2",
        "「🚦 今天」頁**兩張卡的標題就是 raw block key**（`today.actions`／`today.warroom`）。",
        "WC 組實測：這兩張卡的 `title` 欄位**逐字等於它自己的 block key**，"
        "⇒ 畫面上直接印出程式用的鍵名，屬 K1 的同一族（`S2-UI_SPEC.md` S-10）。"
        "⛔ 本輪不修，理由同 K1：規格**沒有給這兩塊的中文標題** —— "
        "`UI_PAGE_TODAY.md` 對這兩塊只寫了版面與狀態，沒寫它在畫面上叫什麼。"
        "⚠️ 另有一層：卡標題**會不會被讀成版面元素**（改標題算不算 UI 異動）"
        "本身未定 ⇒ 若判為異動，還要走客戶草稿先行（`CLAUDE.md §-1.5.D §03-2 ①`）。",
    ),
    (
        "K3",
        "外部編號 **`D-3`** 出現在「💼 我的持股」頁 **6 張卡**的 fact key 上 —— "
        "與客戶點名的編號同族，但**客戶未點名** ⇒ **登記、⛔ 不自行擴表**。",
        "WC 組實測：`D-3` 在該頁卡面共 **7 次、散在 6 張卡**。"
        "它與客戶六詞表裡那個編號（逐字引述：「`D-2`」）、以及本輪授權新增的 `D-08` "
        "是**同一個編號族**（`S2-UI_SPEC.md §9.3` 必出揭露落點表的 `D-n`）。"
        "⛔ **不順手收進詞表的理由**：客戶本輪派工單**逐字列了 `D-08`**、"
        "**⛔ 沒有列 `D-3`** ⇒ 收它就是「自行擴表」，比照本檔對 `D-08` 的既有處置"
        "（2026-09-22 之前：登記、不擴表）。**等客戶一句話即可收進 `_BANNED_PAGE`。**"
        "⚠️ 若客戶裁示收，落點是**頁級**（該編號的揭露落點逐字就寫在那一頁）。",
    ),
    (
        "K4",
        "⭐ 小寫 `l1` 在三頁卡面各 1 次 —— **本組實測它⛔ 不是大寫 `L1` 的小寫**，"
        "而是**線框的「葉1」記法**（下段逐字引述三處上下文）"
        "⇒ 逕自收進詞表會製造**假違規**。",
        "WC 組實測三處上下文（逐字引述卡面）："
        "「🔍 找標的」`find.screen_form`「…內只有這一顆、住預設葉 l1」；"
        "「🔬 查一檔」`inspect.form` 同句；"
        "「💼 我的持股」`hold.scale_disclosure`「規格 ① 逐字：它標 l1 卻列在第○層 ＝ 屬葉」。"
        "⇒ 三處的 `l1` 都指**葉（leaf）編號**，⛔ 不是分層代號。"
        "**大小寫敏感與否規格未定**：客戶寫的是大寫，規格沒有任何一處說明大小寫如何處理。"
        "⇒ 本輪守衛**大小寫敏感**（`esc(w) in one` 直接比對）、**⛔ 不做大小寫折疊** —— "
        "折疊會讓這三處變成違規，而修它們要改線框術語，那是規格的事。**登記、等裁示。**",
    ),
    (
        "K5",
        "🔴🔴 **結構性矛盾（本組認為最重要的一條）**：這五頁原型的**卡面體例本身**"
        "就是「逐字引述規格與線框」，而客戶的「標明『引述』可免責」**只給了附錄／缺口段**，"
        "**卡面沒有這個豁免**。⇒ 本輪修掉的那 8 處，**100% 來自引述**。",
        "實測三件事：(a) 本檔四頁的 `build_*_cards()` 大量使用「線框 x 逐字」「規格 y 逐字」"
        "當 fact key，那是**刻意的體例** —— 它讓讀者查得到每一句的出處；"
        "(b) 客戶 2026-09-22 裁示的兩側處置**不對稱**：卡面全禁／附錄可引述但要標明；"
        "(c) 原型甚至自己在「🔬 查一檔」頁的卡面寫過「⛔ 連引述都不行」。"
        "⇒ **體例與禁令在卡面相撞**：要留出處就會把禁詞印上去，要守禁令就得把出處搬走。"
        "**本輪的處置是把那一張卡的出處搬進附錄**（見缺口 F18／W13），"
        "但那是**一張卡的個案處理**，⛔ 不是對這個體例的通案裁決。"
        "⚠️ **這是體例與禁令的衝突，⛔ 不是誰寫錯** —— WC 組**⛔ 不裁決**，登記給客戶。",
    ),
    (
        "K6",
        "⭐ **禁令的射程有 block／頁／站三級並存，而規格⛔ 無一處統一宣告** —— "
        "本輪的守衛因此改成三級表；**怎麼切是 WC 組拍板，⛔ 非客戶明示**。"
        "🆕 **2026-09-22 補**：原本**同一個縮排形狀被判成兩種級別**（互斥，⇒ 本條⛔ 無法被後人重算）；"
        "WE 組已改寫成**可重算的兩段式判準**，就地寫死在 `_BANNED_SITE` 上方。",
        "三級的出處（逐字引述）：**站級** ＝ 客戶 2026-09-22「卡面：⛔ 禁用內部詞」"
        "（未指定頁）＋ `S2-UI_SPEC.md §9.1`「全站任何位置：標題、卡片、表格…」；"
        "**頁級** ＝ `UI_PAGE_FIND.md ④(d)` 的兩個詞（那是那一頁的規格）；"
        "**block 級** ＝ `UI_PAGE_TODAY.md:74`「⛔ 不得顯示：損益、部位佔比、金額、張數、均價」"
        "—— 該句**縮排在 `- 🆕 today.holdings` 那條 bullet 底下**，射程是那一塊。"
        "🔴 **把 block 級當站級會產生假違規（實測，⛔ 不是推論）**：TODAY 那五詞若讀成站級，"
        "「💼 我的持股」頁立刻 **9 張卡**命中（`張數` 4 卡／`均價` 3 卡／`損益` 2 卡／`金額` 1 卡；"
        "以次計為 6／7／2／2）—— 但該頁帳本模型本來就有那些欄 ⇒ 顯然不是規格的意思。"
        "🆕 **2026-09-22 WE 組補（獨立稽核組 WD 抓到的自相矛盾）**：WD 實測兩條禁令**縮排形狀相同、"
        "歸級卻相反** —— `UI_PAGE_TODAY.md:74`（縮排 2，最近 ind=0 bullet 為 `:64 today.holdings`）"
        "掛 **block**；`UI_PAGE_FIND.md:105`（縮排 2，最近 ind=0 bullet 為 `:103 find.pe_two_kinds`）"
        "掛 **頁**。⇒ 判準各用了一次，**本條因此⛔ 無法被後人重算**。"
        "**處置（⛔ 不縮小任何現有射程）**：改寫成兩段式 —— "
        "**① 預設以規格縮排歸屬為準**；**② 從嚴例外（掛得比 ① 更寬）必須逐項寫明理由**，"
        "且**⛔ 禁止反向**（掛得比 ① 更窄 ＝ 弱化守衛）。"
        "FIND 的 `D-08`／`虧損` 已**明文登記為從嚴例外第一項、維持頁級⛔ 不縮回 block 級**，"
        "理由用規格自己給的那一句（逐字引述 `UI_PAGE_FIND.md:105`：「系統手上沒有 EPS，"
        "說「虧損」＝ 替上游宣稱一件它沒說的事」）—— **該理由對整頁成立**，⛔ 不只對那一塊。"
        "⚠️ **本切法與兩段式判準⛔ 皆未經客戶裁示、⛔ 非規格逐字**，客戶一句話即可推翻；"
        "推翻後只要改 `_BANNED_SITE`／`_BANNED_PAGE`／`_BANNED_BLOCK` 三張表的歸屬。",
    ),
    (
        "K7",
        "`S2-UI_SPEC.md §9.1` 的**站級禁用語七條**（射程逐字含「**卡片**」）"
        "**先前⛔ 沒有任何守衛在守** —— 本輪收進站級表。實測卡面 **0 命中**。",
        "逐字引述該節範圍：「⛔ 禁用語（全站任何位置：標題、卡片、表格、佐證區、提示、"
        "推播、AI 產出）」，七條為：建議買進／立即出清／應該加碼／推薦／必漲／目標價／"
        "＋ 本稿新增的『建議持股 X%』。"
        "⚠️ **加進來仍然安全的理由**：WC 組實測五頁 122 張卡**一次都沒有命中** "
        "⇒ 加了⛔ 不會讓 CI 變紅，但從此**有人守著**。"
        "⚠️ **⛔ 不得把這條讀成「文案已合規」** —— 0 命中只代表「這七個字串不在卡面上」，"
        "§9.1 真正禁的是**那個謂語**（該節逐字：「違規的是那個百分比在第一層的位置，"
        "不是『建議』兩個字」），而**那件事機器掃不出來，本輪也⛔ 沒有人工逐卡判過**。",
    ),
    (
        "K8",
        "**「不得叫／不得標成／不得寫／沒有…按鈕」型的否定式引述共 7 處** —— "
        "它們把禁令印在卡面上，**算不算違規本身未定** ⇒ 登記、⛔ 本輪不判。",
        "WC 組以自己的正則實測 **7 處**（與前組 WB 報的數字一致，但**是各自量的**）："
        "「🔬 查一檔」`inspect.stock.valuation`／`inspect.evi.overlap`；"
        "「💼 我的持股」`hold.run_scope`（逐字引述：「本區沒有停用按鈕」）／"
        "`hold.conclusion_cards`（2 處）／`hold.scenario_calc`／`hold.evi_notready`。"
        "🔴 **分界線是判斷、⛔ 不是規格逐字**：WB 組自陳「違規 vs 模糊的分界是本組判定、"
        "⛔ 非規格逐字」；若客戶裁成**一律從嚴**（連否定式引述也算），"
        "本輪的總數會從 **8 變 19**。⇒ **WC 組⛔ 不判，登記給客戶。**"
        "⚠️ 本組同意 WB 的「這是判斷不是事實」，**但⛔ 沒有獨立複驗 WB 的分界理由本身**。",
    ),
    (
        "K9",
        "📌 **出處更正（只動事實、⛔ 不動語意）**：FIND 的禁詞條在 **④(d)**，⛔ 不是 ⑤。",
        "WC 組實測（逐字引述行號）：`UI_PAGE_FIND.md:105` 那一句"
        "「⛔ 使用者看到的字面不得出現…這類內部詞；⛔ 亦不得出現「虧損」」"
        "落在 **④（`:85` 起「本益比的兩種「沒有」」）** 的 **(d) 現行落地** 小節內；"
        "**⑤ 起於 `:110`，標題是「反例自檢」** ⇒ 舊註寫「⑤ 自己的禁詞條」是**出處寫錯**。"
        "⚠️ **⛔ 不是政策變更**：那一條禁令的內容、射程、效力**一個字都沒有變**，"
        "改的只有「它住在哪一節」。舊註的錯誤已在 `_BANNED_ON_CARD_FACE` 上方就地更正。"
        "⚠️ 本更正是**單組實測**（`grep -n` ＋ 章節起訖行號比對），未經第二組獨立驗。",
    ),
    # ══ 🆕 2026-09-22 WE 組接續登記 K10~K16（⛔ 全部是登記、⛔ 不裁決）══════════
    #    來源：獨立稽核組 WD 稽核 commit `e8d3a75` 的產出 ＋ WE 組本輪自己的實測。
    #    ⚠️ 依客戶 2026-09-22 立的規則逐字：「**模糊地帶：先登記、⛔ 不自行判違規**」。
    (
        "K10",
        "🔴 **`hold.alloc_deviation` 是全站最像 `S2-UI_SPEC.md §9.1` 實質違規的卡，"
        "卻⛔ 不在 K8 的 7 處否定式引述內** —— **兩讀法並陳，WE 組⛔ 不裁決。**",
        "卡面逐字（引述）：fact key「⚠️ 實作端現況」的值為"
        "「現行 label 仍是「80/20 配置偏離（**該減**）」、區段註解仍寫「衛星**停利**」"
        "⇒ **含動作語，登記為合規待處理（須改 code）**」；同一張卡另有"
        "「⛔ 全頁禁止買賣建議 ⇒ 規格 ⑧ 逐字：「達門檻」是**狀態陳述**，"
        "⛔ 不得延伸為「該賣／該加碼」」。"
        "**(甲) 不算違規**：這兩句的謂語是「**現況是什麼**」（它在描述實作端還沒改的字串），"
        "⛔ 不是在對使用者下指令；且同卡已把禁令一併印出來 ⇒ 屬 K8 的否定式引述同族。"
        "**(乙) 算違規**：§9.1 射程逐字「**全站任何位置：標題、卡片、表格、佐證區、提示、"
        "推播、AI 產出**」，且該節自註（逐字引述）「**違規的是那個百分比在第一層的位置，"
        "不是「建議」兩個字**」⇒ 依它自己的標準，**謂語式辯護並不成立**。"
        "⚠️ **WD 抓到的重點是「它不在 K8 那 7 處內」** —— K8 的窮舉用的是"
        "「不得叫／不得標成／不得寫／沒有…按鈕」這組句型正則，而本卡用的是"
        "「仍是／仍寫／不得延伸為」⇒ **句型不同而被漏掉，⛔ 不是被判為合規**。"
        "⇒ **登記給客戶**；若客戶裁成「算」，K8 的總數會再往上加。",
    ),
    (
        "K11",
        "⚠️ **`建議持股 X%` 被收成 `建議持股`（去掉 `X%`）—— 從嚴且合理，"
        "但那是 WC 組的判斷、⛔ 不是規格逐字。** 就地標明。",
        "`S2-UI_SPEC.md §9.1` 的第七條逐字（引述）帶了 `X%` 這個佔位符，"
        "而 `_BANNED_SITE` 收的是**去掉 `X%` 的前四個字**。"
        "**這麼做的理由（WC 組未寫、WE 組補）**：`X%` 是**佔位符⛔ 不是字面**，"
        "照抄進詞表會變成「只有卡面剛好印出 `建議持股 X%` 這七個字才會被抓到」"
        "⇒ **等於沒有守衛**（真違規會寫成「建議持股 30%」）。"
        "⇒ 去掉佔位符是**唯一能真的守到東西**的收法，且**射程只會更寬、⛔ 不會更窄**。"
        "⚠️ **但它確實是一個判斷**：規格沒有說「佔位符要怎麼處理」，"
        "⇒ 依 `CLAUDE.md §-2 規則 6` 就地標明，**⛔ 不得被引用為「規格逐字如此」**。"
        "✅ 實測：本輪五頁 122 張卡對 `建議持股` **0 命中**。",
    ),
    (
        "K12",
        "⚠️ **規格 ④(d) 自己帶了一個 caveat（逐字：「下列擬稿為本組延伸、⛔ 非客戶逐字」），"
        "而卡面的 fact key 只寫「擬稿」二字** —— 算不算帶到了那個 caveat，**兩讀法並陳。**",
        "出處逐字（引述）：`UI_PAGE_FIND.md:103` 括號內「⚠️ 下列擬稿為**本組延伸、"
        "⛔ 非客戶逐字**；參 `page_find.py:553 PE_EMPTY_WHY` …的寫法 ＋ 線框 `live` 現況段」。"
        "卡面實際寫的是 `find.pe_two_kinds` 的 fact key「**就地說明（規格 ④(d) 擬稿）**」。"
        "**(甲) 帶到了**：「擬稿」二字在本檔體例裡本來就等於「非定稿、非客戶逐字」，"
        "且同卡另有 fact key 直接指回規格節號 ⇒ 讀者查得到那個 caveat。"
        "**(乙) 沒帶到**：caveat 的重點是「**這是誰寫的**」（本組延伸 vs 客戶裁示），"
        "而「擬稿」只表達「還沒定稿」⇒ 使用者看不出那段文案的**作者不是客戶**。"
        "⚠️ 依 `CLAUDE.md §-2 規則 6`，「來歷」與「定稿與否」是兩件事 ⇒ (乙) 不能直接駁回。"
        "⛔ **本輪⛔ 不改卡面**：改它會動到卡面文案（本輪的硬約束是卡面 diff ＝ 0），"
        "且「fact key 要不要加來歷」屬**卡面體例**問題 ⇒ 與 K5 同族，**登記給客戶**。",
    ),
    (
        "K13",
        "🔴 **block 級那一條禁令⛔ 不只有五個字面 —— 它真正的射程是一個「概括子句」，"
        "而概括子句⛔ 機器掃不出來。** 本輪就地補揭露（⛔ 不新增守衛，因為守不到）。",
        "`UI_PAGE_TODAY.md:74` 全句逐字（引述）：「⛔ **不得顯示**：損益、部位佔比、金額、"
        "張數、均價，**或任何需要張數／均價才算得出來的量**」。"
        "⇒ `_BANNED_BLOCK['today.holdings']` 收的是**逗號前那五個字面**；"
        "逗號後的概括子句禁的是一個**推導關係**（這個數字是不是靠張數／均價算出來的），"
        "**⛔ 不是一組字串** ⇒ 無法用 `in` 比對守到。"
        "**為何非揭露不可**：`S2-UI_SPEC.md §9.1` 的同型問題已在 K7 末段揭露過"
        "（逐字引述：「真正禁的是**那個謂語**」），**block 級這一條先前⛔ 沒有對應揭露** ⇒ "
        "後人會以為「五個詞 ＝ 全部」。"
        "⚠️ **實例（WE 組舉例，⛔ 非窮舉）**：「總市值」「未實現報酬率」「持股成本」"
        "這些字面**一個都不在五詞內**，但**全部落在概括子句裡** ⇒ 新增欄位時**必須人工判**。"
        "📌 揭露已就地寫在 `_BANNED_BLOCK` 上方（⛔ 不只寫在這裡 —— 加欄位的人不會先來讀附錄）。",
    ),
    (
        "K14",
        "🔴 **`inspect.evi.overlap` 卡面上有一句字面命中 `UI_PAGE_INSPECT.md:86` 的禁令** —— "
        "但它是**否定式引述**，屬 K5／K8 那一族的**未裁決**事項 ⇒ "
        "**⛔ 不進守衛表、⛔ 不動卡面，只登記。**",
        "規格出處 `UI_PAGE_INSPECT.md:86`（引述；⚠️ **原句開頭那三個字與本檔"
        "「📖 憑什麼」頁的守衛相撞** ⇒ 此處**只引不含那三字的後半**，比照本檔對 HOLD "
        "states 的既有處置「一律改引不含那三字的片段」，**⛔ 不是改規格**）："
        "「…的設計理由揭露，不是資料功能。⛔ 不得寫成、也不得實作成"
        "「這檔股票被哪些 ETF 持有」。」（被省略的前半語意 ＝「它是『兩種標的為何共用一頁』」）"
        "卡面逐字（引述）：fact key「⛔ 它不是資料功能」，值為"
        "「規格 ③③ 逐字：它是「兩種標的為何共用一頁」的設計理由揭露，"
        "⛔ 不得實作成「這檔股票被哪些 ETF 持有」」。"
        "🔴 **判定（AI 總管已複驗，WE 組照抄）**：那是**否定式引述** —— "
        "它在告訴使用者這一頁**刻意不做**這件事，**⛔ 不是宣稱這個功能存在**。"
        "⇒ 它落在 WC 組已登記的 **K5（卡面體例本身就是逐字引述）／K8（否定式引述分界未判）** "
        "那一族，屬**未裁決**事項。"
        "⚠️ 依客戶 2026-09-22 立的規則逐字（引述）：「**模糊地帶：先登記、⛔ 不自行判違規**」"
        "⇒ **⛔ 不收進 `_BANNED_BLOCK`**（收了會立刻變紅，而唯一能讓它變綠的作法是改卡面文案"
        "—— 那是本末倒置，且落在客戶的草稿先行射程內）。"
        "📌 **同一次掃描的另外兩條（`UI_PAGE_TODAY.md:76`／`:249` 的「完美分散」）"
        "卡面 0 命中 ⇒ 已收進 `_BANNED_BLOCK`**，本條是三條裡唯一不收的。",
    ),
    (
        "K15",
        "⚠️ **產物檔頭的「產生當下 HEAD」永遠是父 commit** —— 結構使然、⛔ 非缺陷；"
        "但任何人拿它判「這份 HTML 由哪個 commit 產生」會**差一個 commit**。",
        "成因（⛔ 不是 bug）：`main()` 在**產生的當下**跑 `git rev-parse HEAD`，"
        "而那一刻本輪的改動**還沒有 commit** ⇒ 記下來的必然是**上一個** commit。"
        "⇒ 產物與它自己的 commit 之間**恆差一格**，**⛔ 無法在產生器內部自我修正**"
        "（要記下自己的 hash，得先知道包含自己的 commit 的 hash —— 循環）。"
        "**可行的解法有兩種，⛔ 兩種都不在本輪授權內**："
        "(a) 檔頭改寫成「**產生時的父 commit**」這種**不會被誤讀**的措辭"
        "（⇒ 會動到產物檔頭，但⛔ 不動卡面）；"
        "(b) 改由 commit hook／CI 在 commit 之後回填（⇒ 動到 repo 流程，不只動這兩個檔）。"
        "⚠️ **WE 組⛔ 不自行改**：本輪硬約束是只動產生器與產物、且⛔ 不擴大範圍 ⇒ **登記**。"
        "📌 **現況實測**：`d6633cf` 版產物檔頭記的是 `43c3228`（＝ `e8d3a75` 的父）。",
    ),
    (
        "K16",
        "⚠️ **「規格裡只有這 3＋1 條漏收」是獨立稽核組 WD 的單組、單一方法全稱句，"
        "⛔ 不得被引用為「已窮舉」** —— WD 自陳的盲點，WE 組據實轉錄。",
        "WD 的方法：**縮排歸屬 ＋ 正則**，掃的範圍是 `UI_PAGE_TODAY.md`／`UI_PAGE_FIND.md`／"
        "`UI_PAGE_INSPECT.md`／`UI_PAGE_HOLD.md`／`UI_PAGE_WHY.md` ＋ `S2-UI_SPEC.md`。"
        "**⛔ 完全沒掃的**：`S1-3B`、線框 `.js`、`DECISION_RULES.md`、`INDICATOR_SPEC.md`。"
        "⇒ 依 `CLAUDE.md §-2 規則 6`，「規格裡只剩這幾條沒收」**只能當待驗事項**，"
        "⛔ 不得作為後續動作的前提、⛔ 不得寫進 commit message 當成已完成的事實。"
        "⚠️ **WE 組同樣沒有補掃那四處**（本輪授權只到修 WD 點名的 6 項）⇒ "
        "**這個盲點在本輪之後仍然存在**，⛔ 不是「已處理」。"
        "📌 **要消掉它需要的是另派一組獨立窮舉**（`CLAUDE.md §-2 規則 4`：調查／稽核須多組視角、"
        "⛔ 不可由想動手的那一組自己查）。",
    ),
)

#: 六組缺口（組名 → 條目）。**畫在畫面上的順序與這裡無關**（各頁自己畫自己的）；
#: 這份只給 `main()` 的守衛用：條數下限／編號不重複／缺口標記計數／「引述」標註。
#: ⚠️ 2026-09-22 起 **六組**（原五組 ＋ 新的 `COPY_SPEC_GAPS`）。
_ALL_GAPS: tuple[tuple[str, tuple[tuple[str, str, str], ...]], ...] = (
    ("CHROME_SPEC_GAPS", CHROME_SPEC_GAPS),
    ("COPY_SPEC_GAPS", COPY_SPEC_GAPS),
...
#: ⭐ **2026-09-22 由客戶派工單收斂；上面兩段刪除線是有意識的政策變更，⛔ 不是漏刪**
#:    （決策者 **客戶**，執行 WC 組）。**舊句的理由仍然成立**：它精準記錄了
#:    「客戶裁示當下只知道一處、所以刻意不自行擴表」這個**歷史生成過程**，
#:    讀 2026-09-22 之前的 commit 時仍應照此理解。**被權衡掉的是它的兩個前提** ——
#:    (a) 客戶本輪派工單**逐字把 `D-08` 列進窮舉清單** ⇒「要不要收」已由客戶決定；
#:    (b) 「共 7 處」這個數字本身**當時就漏了一處**：`UI_PAGE_FIND.md ④(d)` 逐字另寫
#:        「⛔ **亦不得出現「虧損」**」，而前幾輪全部漏掉它 ⇒ 實為 **8 處**。
#:    ⚠️ 另據實更正一處**出處**（⛔ 不是語意變更）：FIND 的禁詞條在 **④(d)**
#:    （`UI_PAGE_FIND.md:105`，④ 起於 `:85`、⑤ 起於 `:110`），**⛔ 不是 ⑤**
#:    —— ⑤ 是反例自檢。舊註寫「⑤」，本輪實測更正，登記於缺口 **K9**。
#: ⚠️ 這份清單**同時**被「附錄／層註必須標明『引述』」那道守衛（⑥-b）使用 ——
#:    同一組詞、**兩種處置**：卡面全禁／附錄可引述但要標明。
#:    🔴 **⑥-b 的詞表本輪⛔ 一個字都沒有動**（仍是下面這六詞）—— 動它會讓
#:    「附錄要標『引述』」的射程外擴到 `D-08`／`虧損`／站級禁用語，那是**另一件事**，
#:    ⛔ 不在客戶本輪授權內。**⛔ 不動 ≠ 弱化**：六詞一個未減，只是沒有加。
_BANNED_ON_CARD_FACE: tuple[str, ...] = (
    "加權移動平均", "FIFO", "先進先出", "D-2", "L1", "旗標",
)

#: ══ 卡面禁詞的**三級射程**（🆕 2026-09-22 WC 組新增；⛔ 純新增，無舊表述被覆蓋）══
#:
#: 🔴 **為什麼需要「射程」這個概念**（⛔ 不是為了設計得漂亮）：
#:    規格裡的禁令**本來就有三種射程**，而規格**沒有任何一處統一宣告過這件事** ——
#:    - **站級**：客戶 2026-09-22 裁示「卡面：⛔ 禁用內部詞」**沒有指定頁**；
#:      `S2-UI_SPEC.md §9.1` 逐字「**全站任何位置：標題、卡片、表格、佐證區、提示、推播、
#:      AI 產出**」，射程逐字含「卡片」。
#:    - **頁級**：`UI_PAGE_FIND.md ④(d)` 的 `D-08`／`虧損` 是**那一頁那一塊的規格**。
#:    - **block 級**：`UI_PAGE_TODAY.md:74`「⛔ 不得顯示：損益、部位佔比、金額、張數、均價」
#:      **縮排在 `- 🆕 today.holdings` 這一條 bullet 底下**（`:64` 起），
#:      ⇒ 它的射程是**那一塊**，⛔ 不是那一頁、⛔ 更不是全站。
#: 🔴 **把頁級／block 級的詞當站級用，會產生假違規**（本輪實測，⛔ 不是推論）：
#:    把 TODAY 那五詞讀成站級 ⇒ 「💼 我的持股」頁立刻 **9 張卡**命中
#:    （`張數` 4 張卡／`均價` 3 張卡／`損益` 2 張卡／`金額` 1 張卡；⚠️ 以**卡**計，
#:    以**次**計為 `張數` 6／`均價` 7／`損益` 2／`金額` 2）——
#:    但該頁的帳本模型本來就有 `lots`／`avg_price` 欄（`UI_PAGE_HOLD.md`），
#:    ⇒ 那顯然**不是**規格的意思。**扁平表做不出這個區分，所以本輪改成三級。**
#: ⚠️ **本切法是 WC 組的內部拍板（`CLAUDE.md §-1.5.D §03-1`「代碼與資料修復」），
#:    ⛔ 非客戶明示、⛔ 非規格逐字** —— 客戶一句話即可推翻。登記於缺口 **K6**。
#: ⚠️ **三級一律只作用在卡面**，⛔ 不作用在附錄／層註（那一側由 ⑥-b 管）。
#:
#: ⭐⭐ **歸級判準（2026-09-22 WE 組就地寫死；⛔ 純新增，無舊表述被覆蓋）** ⭐⭐
#: 🔴 **為什麼非寫不可**：獨立稽核組 WD 實測出**兩條互斥的判準各被用了一次** ——
#:    `UI_PAGE_TODAY.md:74`（縮排 2，最近的 ind=0 bullet 是 `:64 today.holdings`）掛 **block**；
#:    `UI_PAGE_FIND.md:105`（縮排 2，最近的 ind=0 bullet 是 `:103 find.pe_two_kinds`）掛 **頁**。
#:    ⇒ 同一個縮排形狀得到兩種歸級，**K6 因此⛔ 無法被後人重算**。
#:    （WD 的實測已由 WE 組在 TODAY／FIND 兩端各自複驗，**⛔ 未經第三組驗**。）
#: **① 預設規則（可重算）**：**以規格的縮排歸屬為準** ——
#:    禁令行若縮排在某一條 ind=0 的 block bullet 底下 ⇒ **block 級**；
#:    若該 bullet 本身不是單一 block（例如「第二層：…」帶三個 block）⇒ 取**句子語意所指的 block**，
#:    並依 ② 標明；若禁令行本身就在 ind=0 ⇒ **頁級**；若出處是站級規格（`S2-UI_SPEC.md`）⇒ **站級**。
#: **② 從嚴例外（掛得比 ① 更寬）**：**允許，但⛔ 必須逐項寫明理由** ——
#:    格式為「〈詞〉→〈實掛級別〉（① 會判〈級別〉）：〈理由〉」。
#:    ⚠️ **⛔ 不准反向** —— 掛得比 ① **更窄**（block 級規格只掛那一塊以外、或頁級縮成 block 級）
#:    一律禁止：那是**弱化守衛**，不是判準選擇。
#: **③ 現存的從嚴例外，逐項列出（本表即唯一落點，⛔ 不另開第二份）**：
#:    - **`D-08`／`虧損` → 頁級（① 會判 block 級 `find.pe_two_kinds`）**：
#:      理由取自規格自己給的那一句 —— `UI_PAGE_FIND.md:105` 逐字
#:      「⛔ 亦不得出現「虧損」（線框 `evidence` 已定：系統手上沒有 EPS，
#:      說「虧損」＝ 替上游宣稱一件它沒說的事）」。**「系統手上沒有 EPS」這個前提對整頁成立**，
#:      ⛔ 不是只對那一塊成立 ⇒ 掛頁級才對得上理由。`D-08` 是同一句裡並列的內部詞，同級處理。
#:      ⚠️ **⛔ 不得為了「讓判準一致」而把它縮回 block 級** —— 那是弱化（見 ②）。
#:    - **「完美分散」→ 同時掛 `today.holdings` 與 `today.summary`（① 對 `:76` 判前者、
#:      對 `:249` 判後者）**：理由 —— 規格兩處都寫了這個禁令
#:      （`UI_PAGE_TODAY.md:76`「⛔ 不得顯示 `0` 或「完美分散」」縮排在 `:64 today.holdings` 底下；
#:      `:249`「⛔ 不得顯示「完美分散」或 `0`」縮排在 `:247`「第二層：…」底下，
#:      而 `:247` 那條 bullet **同時帶 `today.summary`／`today.key_banner`／`today.holdings` 三塊**）
#:      ⇒ 依 ① 的第二子句，`:249` 的**語意**指向 `today.holdings`（句子主詞是 `:248` 的它），
#:      但**縮排最近的 ind=0 bullet 首個 block key 是 `today.summary`** ⇒ 兩讀法並存。
#:      **兩塊都掛 ＝ 取聯集 ＝ 從嚴**，⛔ 不在兩個讀法之間挑一個而讓另一個沒人守。
#: ⚠️ **本判準是 WE 組拍板，⛔ 非客戶明示、⛔ 非規格逐字** —— 登記於缺口 **K6**（已改寫）。

#: 站級：五頁卡面一律禁。＝ 客戶六詞 ∪ `S2-UI_SPEC.md §9.1` 禁用語 ∪ `S2-UI_SPEC.md:343` 缺值字面。
#: ⚠️ §9.1 那七條**本輪實測卡面 0 命中** ⇒ 加進來**不會讓 CI 變紅**，
#:    但從此**有人守著**（⛔ 不是「以後靠自律」）。登記於缺口 **K7**。
#: 🆕 **2026-09-22 WE 組新增四個缺值字面**（`None`／`nan`／`NaT`／`inf`）——
#:    出處 `S2-UI_SPEC.md:343` 逐字：「**⇒ 本份只下一條規格**：畫面上任何一格⛔
#:    **不得出現字面 `None`／`nan`／`NaT`／`inf`**」。射程逐字「**畫面上任何一格**」⇒ **站級**。
#:    ✅ **實測**：加進來之後五頁 122 張卡**仍 0 命中**（WE 組以與守衛 ⑯ 相同的
#:    `esc(w) in card_html(...)` 比對方式跑過）⇒ ⛔ 不會讓 CI 變紅。
#:    ⚠️ **已知脆弱處，據實寫出（⛔ 不是「應該沒問題」）**：這四個是**短 ASCII 字串**，
#:    ⇒ 日後若有卡面出現 `information`／`infra`／`nanometer`／`None` 這類**誤命中**，
#:    處置**一律是加詞邊界或改精準比對**，
#:    ⛔ **不得**把詞從表裡拿掉（弱化）、⛔ **更不得**為了讓 CI 變綠去改卡面文案（本末倒置）。
_BANNED_SITE: tuple[str, ...] = _BANNED_ON_CARD_FACE + (
    "建議買進", "立即出清", "應該加碼", "推薦", "必漲", "目標價", "建議持股",
    # 🆕 `S2-UI_SPEC.md:343`（站級，逐字「畫面上任何一格」）。
    "None", "nan", "NaT", "inf",
)

#: 頁級：只在該頁的卡面禁。key ＝ `shared.ia_nav` 的頁 id（⛔ 不手抄字串）。
#: ⚠️ `D-08`／`虧損` 的出處是 `UI_PAGE_FIND.md ④(d)` ⇒ **那是「🔍 找標的」頁的規格**，
#:    故掛頁級、⛔ 不掛站級 —— 把一頁的規格套到別頁去就是**自行擴表**。
#:    （`D-08` 經客戶本輪派工單逐字授權；`虧損` 是該條逐字明文。）
#: 🆕 **2026-09-22 WE 組補**：這兩個詞是上方判準 ③ 的**從嚴例外第一項**
#:    （① 的縮排規則會判 block 級 `find.pe_two_kinds`）——
#:    **維持頁級、⛔ 不縮回 block 級**，理由見上方 ③，⛔ 不在這裡重寫一份。
#: ⚠️ **同族的 `D-3` ⛔ 刻意不在此表**：它在「💼 我的持股」頁 6 張卡的 fact key 上，
#:    與 `D-2`／`D-08` 同一編號族，但**客戶未點名** ⇒ 比照既有對 `D-08` 的處置，
#:    **登記、⛔ 不自行擴表**（缺口 **K3**）。
_BANNED_PAGE: Mapping[str, tuple[str, ...]] = {
    PAGE_FIND: ("D-08", "虧損"),
}

#: block 級：只在該 block 的卡面禁。
#: ⚠️ 只收**規格縮排確實掛在某一個 block 底下**的禁令（見上方 `UI_PAGE_TODAY.md:74` 的判讀
#:    ＋ 歸級判準 ①）。
#: 🔴 **`UI_PAGE_TODAY.md:74` 那一句⛔ 不只有這五個詞 —— 機器只守得到字面那半，據實寫出**
#:    （2026-09-22 WE 組就地揭露；登記於缺口 **K13**）：
#:    該行全句逐字為「⛔ **不得顯示**：損益、部位佔比、金額、張數、均價，
#:    **或任何需要張數／均價才算得出來的量**」。
#:    ⇒ 逗號後面那個**概括子句**（「或任何需要張數／均價才算得出來的量」）
#:    **是這條禁令真正的射程**，而它**⛔ 機器掃不出來** —— 它禁的是一個**推導關係**
#:    （這個數字是不是用張數／均價算出來的），⛔ 不是一組字串。
#:    ⚠️ **⛔ 不得把「這五個詞沒命中」讀成「這一塊已合規」** —— 新增欄位時
#:    **必須人工判**它是不是需要張數／均價才算得出來（例如「總市值」「報酬率」「持股成本」
#:    這些字面都不在五詞內，但全部落在概括子句裡）。
#:    📌 同型問題在站級已有先例揭露：§9.1 真正禁的是**那個謂語**（見缺口 **K7** 末段）——
#:    **block 級這一條先前⛔ 沒有對應揭露**，本輪補上。
#: 🆕 **2026-09-22 WE 組新增「完美分散」**（出處 `UI_PAGE_TODAY.md:76` 與 `:249`，逐字見上方
#:    判準 ③ 第二項）—— **兩塊都掛 ＝ 從嚴取聯集**。
#:    ✅ **實測**：加進來之後五頁 122 張卡 **0 命中** ⇒ ⛔ 不會讓 CI 變紅，但從此有人守著。
_BANNED_BLOCK: Mapping[str, tuple[str, ...]] = {
    "today.holdings": ("損益", "部位佔比", "金額", "張數", "均價", "完美分散"),
    # 🆕 `UI_PAGE_TODAY.md:249`（縮排掛在 `:247`「第二層」那條 bullet 底下）。
    "today.summary": ("完美分散",),
}

#: ⭐ **key 的結構守衛**（2026-09-22 WE 組新增；⛔ 純新增，無舊表述被覆蓋）。
#: 🔴 **為什麼需要（⛔ 不是為了多一道保險）**：`banned_words_for()` 取詞用的是
#:    `_BANNED_PAGE.get(page_id, ())` 與 `_BANNED_BLOCK.get(block, ())` ——
#:    **`.get` 找不到 key 就回空 tuple，⛔ 不報錯**。
#:    ⇒ key 打錯、或那一塊日後**改名／被拆開**，那一組禁詞會**靜默失效而 CI 全綠**。
#:    ⚠️ 這**正是本輪的病根同一型**：守衛 ⑯ 已經替「**頁**」做了集合比對
#:    （`{p for p, _, _ in _PAGE_CARD_SOURCES} == set(PAGE_LABELS)`，就是為了防這件事），
#:    **卻沒有替「block」做** ⇒ 五個 block 級禁詞一直沒有結構守衛。本處補齊。
#: ⚠️ **block key 的全集一律從既有的五份 LAYOUT／BLOCK_COLS 結構算出來，⛔ 不手抄第二份清單**
#:    —— 手抄 ＝ 第二真相源（`CLAUDE.md §2.1`），而本輪稍早才剛消滅過一個
#:    （HOLD 葉表併進 `HOLD_LAYOUT`）。
#: ⚠️ 這兩道是**結構**守衛（key 在不在），**⛔ 不是**「這一塊真的被掃到」——
#:    後者由 `main()` 的 ⑯-c 在跑完卡面之後另外驗（LAYOUT 有、但 builder 沒吐出來的洞）。
_ALL_BLOCK_KEYS: frozenset[str] = (
    frozenset(page_today.BLOCK_COLS)
    | frozenset(WHY_BLOCK_COLS)
    | frozenset(FIND_BLOCK_COLS)
    | frozenset(INSPECT_BLOCK_COLS)
    | frozenset(HOLD_BLOCK_COLS)
)
assert set(_BANNED_PAGE) <= set(PAGE_LABELS), (
    f"_BANNED_PAGE 的 key 不是真的頁 id：{sorted(set(_BANNED_PAGE) - set(PAGE_LABELS))} —— "
    "`banned_words_for()` 用 `.get(page_id, ())` 取詞，key 打錯會**靜默失效而 CI 全綠**。"
    "⛔ 不得靠「我看過，拼對了」；頁 id 一律取自 `shared.ia_nav`，⛔ 不手抄字串。")
assert set(_BANNED_BLOCK) <= _ALL_BLOCK_KEYS, (
    f"_BANNED_BLOCK 的 key 不是真的 block：{sorted(set(_BANNED_BLOCK) - _ALL_BLOCK_KEYS)} —— "
    f"五頁 block 全集共 {len(_ALL_BLOCK_KEYS)} 個（由 page_today.BLOCK_COLS ＋ 四份 "
    "*_BLOCK_COLS 算出，⛔ 非手抄）。"
    "key 打錯或那一塊改名／被拆開 ⇒ 該組 block 級禁詞**靜默失效、⛔ 不報錯、CI 全綠**。")


def banned_words_for(page_id: str, block: str) -> tuple[str, ...]:
    """這一頁這一塊的卡面禁詞 ＝ 站級 ∪ 頁級 ∪ block 級。

    ⛔ 刻意**不去重**成 set：順序穩定，錯誤訊息才可重現（§5 可重現性）。
    """
    return (_BANNED_SITE
            + _BANNED_PAGE.get(page_id, ())
            + _BANNED_BLOCK.get(block, ()))
...
def build_gaps_html() -> str:
    """把 `CHROME_SPEC_GAPS` ＋ `COPY_SPEC_GAPS` 畫成可見的一段（**⛔ 不只留在 .py 裡**）。

    ⚠️ 2026-09-22 起本段畫**兩組**（原只有 chrome 那組）——
    新的 `COPY_SPEC_GAPS`（前綴 `K`）記的是「卡面禁詞這件事本身」跨五頁的狀況，
    **⛔ 不屬於任何一頁**，故與五頁共用的 chrome 缺口同段。
    ...
    for gap_id, title, detail in CHROME_SPEC_GAPS:
        ...
        parts.append(f'<p class="pv-meta">{esc(detail)}</p>')
    # 🆕 2026-09-22：第六組「卡面文案與禁詞射程」（前綴 K）同樣畫在共用附錄段 ——
    #    它記的是「禁詞這件事本身」跨五頁的狀況，⛔ 不屬於任何一頁。
    #    ⚠️ 自己的前言必須**也**帶一次 GAP_CAVEAT：main() 的計數式是
    #       `sum(len(group) + 1)`，少一次就當場炸（那正是它要防的事）。
    parts.append(
        '<p class="pv-disclosure">'
        # 🆕 條數**由 `len()` 導出，⛔ 不手寫** —— 舊版寫死「九條」，加一條就過期
        #    （`CLAUDE.md §8.2.A.0 規則 4`：會漂移的量測值一律不寫死）。
        + esc(f"卡面文案與禁詞射程（2026-09-22 新增；本組 {len(COPY_SPEC_GAPS)} 條"
              "⛔ 全部是「登記、不修」）：")
        + "</p>")
    _copy_caveat = (
        "每一條都是：" + GAP_CAVEAT + "。"
        "⛔ 不得被讀成「已判定它們是違規」—— "
        "客戶本輪只授權修 8 處字面 ＋ 補守衛，本組各條皆未經裁示。")
    parts.append(f'<p class="pv-meta">{esc(_copy_caveat)}</p>')
    for gap_id, title, detail in COPY_SPEC_GAPS:
        parts.append(
            f'<p class="pv-meta"><span class="pv-mono">{esc(gap_id)}</span>　'
            f"{esc(title)}　{esc(GAP_CAVEAT)}</p>"
        )
...
def main() -> None:
    ...
    #         📌 **2026-09-22 事實更正 —— 有意識的更正，⛔ 不是漏刪**
    #            （決策者 **AI 總管**，執行 **WE 組**；由獨立稽核組 **WD** 抓到）。
    #            **現行**：卡面那一側是**全面禁止**，由 **⑭-h／⑮-h／⑯ 三道**逐張卡的守衛管，
    #            ⛔ 不是這一道。
    #            - **舊句在它寫下的當天是對的**：那時逐張卡的守衛真的只有兩道
    #              （⑭-h 管「🔬 查一檔」、⑮-h 管「💼 我的持股」）。
    #            - **它自 commit `e8d3a75` 起不成立**：同一輪新增了 ⑯，且射程是**五頁**
    #              ——「兩道」這個數字與「只有兩頁被掃」這個隱含前提**同時失效**。
    #            - ⚠️ **這是事實更正，⛔ 不是政策變更**：作用域那一句
    #              （⑥-b 只管附錄／層註、⛔ 不管卡面）**一個字都沒有被弱化，仍然完全有效**。
    ...
    # ══ ⑯ 🔴 **五頁統一的卡面禁詞守衛**（2026-09-22 WC 組新增）══════════════════
    #
    # 🔴 **為什麼要有這一道（⛔ 不是為了多一道保險，是因為病根在這裡）**：
    #    在本輪之前，逐張卡的禁詞守衛**只有 ⑭-h（🔬 查一檔）與 ⑮-h（💼 我的持股）兩處**
    #    ⇒ **🚦 今天／📖 憑什麼／🔍 找標的 三頁根本沒有人在掃**。
    #    ⚠️ **現存違規全部落在後兩頁，⛔ 不是因為那兩頁比較髒，是因為只有那兩頁沒人看。**
    #    這正是 CLAUDE.md §-2「沒查證的宣稱比沒有宣稱更危險」的機器版失效模式：
    #    **沒有守衛的地方，CI 會一直保持全綠。**
    #
    # 🔴 **⛔ 刻意不刪 ⑭-h／⑮-h**（它們與本道重疊）—— **拿掉一道守衛就是弱化**，
    #    沿用本檔 ⑮ 對 `_HOLD_BLOCK_LEAF` 那道恆真 assert 的同一條紀律。
    #    本道是**加嚴 ＋ 補齊**：⑭-h／⑮-h 用六詞扁平表，本道用**三級射程表**（詞更多）。
    #
    # 🔴 **結構守衛（防同一個病重犯）**：下面第一道 assert 釘住
    #    「**`_PAGE_CARD_SOURCES` 必須涵蓋 `PAGE_LABELS` 的每一頁**」——
    #    ⛔ 不是數「有五頁」，而是比對**頁 id 集合**。日後加第六頁而忘了掛進來，
    #    這一行會當場炸，⛔ 不會像本輪一樣默默漏掉三頁而 CI 全綠。
    #
    # ⚠️ **只掃卡面，⛔ 不掃全檔**：附錄與層註是**內部揭露**，可引述但須標「引述」
    #    （那一側由 ⑥-b 管，本輪⛔ 未動它一個字）。
    _PAGE_CARD_SOURCES: tuple[tuple[str, object, object], ...] = (
        (PAGE_TODAY, build_cards, contextlib.nullcontext),
        (PAGE_WHY, build_why_cards, why_page_contract),
        (PAGE_FIND, build_find_cards, find_page_contract),
        (PAGE_INSPECT, build_inspect_cards, inspect_page_contract),
        (PAGE_HOLD, build_hold_cards, hold_page_contract),
    )
    assert {p for p, _, _ in _PAGE_CARD_SOURCES} == set(PAGE_LABELS), (
        "卡面禁詞守衛沒有涵蓋全部的頁："
        f"{sorted(set(PAGE_LABELS) ^ {p for p, _, _ in _PAGE_CARD_SOURCES})} —— "
        "⛔ 不得讓任何一頁的卡面沒有人掃（本輪的病根就是三頁沒人掃而 CI 全綠）")

    _face_hits: list[str] = []
    _face_seen: dict[str, int] = {}
    _face_blocks: set[str] = set()
    for _pid, _builder, _ctx in _PAGE_CARD_SOURCES:
        with _ctx():                                      # type: ignore[operator]
            _snap = _builder()                            # type: ignore[operator]
            _face_seen[_pid] = sum(len(v) for v in _snap.values())
            _face_blocks |= {_b for _b, _cs in _snap.items() if _cs}
            for _block, _cards in _snap.items():
                for _card in _cards:
                    _one = markup.card_html(block=_block, **_card)
                    for _w in banned_words_for(_pid, _block):
                        if esc(_w) in _one:
                            _face_hits.append(
                                f"{PAGE_LABELS[_pid]} / {_block} / "
                                f"{_card['title']} ⇒ 「{_w}」")
    assert not _face_hits, (
        f"卡面出現禁詞（五頁統一守衛 ⑯）：{_face_hits}\n"
        "—— 客戶 2026-09-22 裁示逐字：卡面⛔ 禁用內部詞；"
        "附錄／缺口段可引述但**必須標明「引述」**。\n"
        "射程三級（站／頁／block）見 `banned_words_for()` 上方與缺口 K6；"
        "⛔ 不得為了讓它變綠而把詞從表裡拿掉（那是弱化，⛔ 不是修）。")
    #    ⑯-b 每一頁都**真的掃到卡**（⛔ 不是「迴圈跑完了」——空 dict 也會跑完）。
    assert all(_n > 0 for _n in _face_seen.values()), _face_seen
    #    ⑯-c 🆕 **每一個 block 級禁詞的 key，都必須真的被上面這一圈掃到**
    #         （2026-09-22 WE 組新增）。
    #         🔴 **它補的是模組層那兩道 assert 補不到的洞**：模組層驗的是
    #            「key 在不在 LAYOUT 的 block 全集裡」（結構），但 LAYOUT 有、而
    #            `build_*_cards()` **沒有吐出那一塊的卡**時，該組禁詞一樣**一次都不會被比對**
    #            —— 結構守衛全綠、實際守衛失效。⇒ 這一道釘的是「**真的掃到了**」。
    #         ⚠️ 只收**有卡**的 block（`if _cs`）：空 list 的 block 等於沒掃。
    assert set(_BANNED_BLOCK) <= _face_blocks, (
        f"_BANNED_BLOCK 有 key 沒被任何一頁掃到："
        f"{sorted(set(_BANNED_BLOCK) - _face_blocks)} —— "
        "那一組 block 級禁詞**一次都沒有被比對過**（⛔ 不報錯、CI 全綠）。"
        "⛔ 不得靠「它在 LAYOUT 裡所以應該有掃到」。")
