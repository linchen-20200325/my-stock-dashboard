...
| **說明文件** | `S1-3_IA_WIREFRAME.md` `S1-3B_FULL_DRAFT_SPEC.md` `UI-A_page_today_wireframe.md` `UI-B_wireframe.md` `UI-C_wireframe.md` `UI-E_page3_wireframe.md` | 文字線框與內容規格（**不是視覺稿、不含前端代碼**） |

### ⭐ `UI-A_page_today_wireframe.md` —— 只有 7 行，但**別刪**

它的正文當初是走回報訊息、沒有落檔，所以檔案本身只是存根。
**但那 7 行裝著整輪「文案瘦身」工作的原始量測**（量測日 2026-09-08，
基準 `7cf3157`，streamlit 1.59.2，AppTest 冷啟動實跑）：

> 頁1 冷啟動 **12,075 字**（葉1 3,123 / 葉2 8,952）· **164 個文字區塊** · **重複句佔 60.7%**
> 方案 A 只動字 ≈ 5,100｜方案 B 加下鑽 ≈ 2,400（可見）｜方案 C 重設計冷態 ≈ 900

這組數字是**客戶本人最初交接時點名的那個問題的原始量測**（他逐字講過
「頁1 冷啟動 12,075 字、同一句話印 34 次」）。
⇒ **它是後續所有瘦身成果的證據地基** —— 沒有它，後人只會看到一堆「瘦身後」的數字，
**找不到「當初到底多糟」**。本存檔組實測：這四組數字在本批其他文件中**0 命中**
（掃 `12,075` / `60.7` / `164 個文字區塊` 三個字串，量測日 2026-09-15，**單組檢查**）。
...
## 3. `wf_onboarding.js` 不在已發布的成品裡 —— **這是刻意的**
...
- 對照實驗：把 `wf_onboarding.js` 加進 `FILES` 後重組，得到
  **9 頁 / 146 區塊**（多出 `onboarding(6)`），與已發布版本不同。

**為什麼不納入（決策者：AI 總管，2026-09-15）**：

> 客戶明令「**同意 UI 設計前不得修改草稿**」，而新手引導是**一個尚未經客戶核准的新分頁**。
> 總管因此**刻意**未將 `wf_onboarding.js` 納入組裝清單 ——
> 納入＝**新增分頁**＝必須先過客戶的草稿 gate。

⇒ 對齊 `CLAUDE.md §-1.5`（v3 §03-2 ①）：版面佈局／欄位增減／**分頁動線異動**前，
**必須先提文字線框草稿供客戶拍板**；且依 `§-1`，**沒有客戶指派就不要動工**。

⛔ **所以「它沒被組進去」不是 bug，不要順手修掉。** 檔案留在本目錄是為了**保住草稿本身**
（客戶哪天拍板就能直接加進 `FILES`），不是等著誰把它補進組裝清單。
