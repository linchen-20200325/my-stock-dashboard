...
> ⚠️ **已登記的唯一例外（2026-09-23，決策者 user）**：**💼 我的持股頁的洞 ⛔ 不在本檔** —— 依客戶當輪範圍裁示，壓在 `docs/v2/spec/DATA_MAP_HOLD.md` **檔末一節**，本檔因此**無 `## 💼 我的持股頁` 分節**（**⛔ 刻意不補、⛔ 不是漏做**）。⚠️ **上面那條紀律本身 ⛔ 未被撤銷、仍然完全有效** —— 這是**一個有登記的例外**，⛔ 不是紀律失效；⛔ 不得被引用來為「其他頁也各留一份」背書。
> ⚠️ **2026-09-23 事實更正，⛔ 不是漏刪**；執行 **GD 更正組**。**現行：五份逐頁對照表已全數製作完成，⛔ 無一份「尚未製作」** —— `DATA_MAP_TODAY.md`（🚦 今天）／`DATA_MAP_FIND.md`（🔍 找標的）／`DATA_MAP_WHY.md`（📖 憑什麼）／`DATA_MAP_INSPECT.md`（🔬 查一檔，**22 block**，commit `0928c75` 產出）／`DATA_MAP_HOLD.md`（💼 我的持股，**23 block**，commit `f89093d` 產出）。⚠️ **五個檔名一個都沒有少** —— 上面那句同時是本檔的**檔案指標**（「對照表本體住在哪一個檔」），⛔ 不得簡寫成「都做完了」而讓檔名消失。⚠️ **行數 ⛔ 不寫在本檔**（會漂移的量測值，`CLAUDE.md §8.2.A.0` 規則 4）—— 行數的 SSOT ＝ `docs/v2/spec/DATA_MAP_ALL_PAGES.md` 索引表，需要時現場 `wc -l`。
> **兩邊理由並陳**：**舊句在它寫下的當下逐字為真、⛔ 不是寫錯** —— 本檔頭自陳量測日 **2026-09-22**，當天 🔬 查一檔與 💼 我的持股**兩檔確實都還沒產**，該句據實登記了那個事實、⛔ 未先假裝它們已存在。**被推翻的只有「這兩檔尚未製作」這個前提，⛔ 不是它承載的判斷** —— 「**對照表本體住在逐頁的檔、洞只留這一份、⛔ 兩邊不各留一份**」這條紀律**一字未減，仍然完全有效**。⚠️ **本行本身也是會漂移的量測值**：往後若新增第六頁，「已全數製作完成」即刻失效 ⇒ **請現場重數，⛔ 不要引用本行當往後的事實**。
...
> 素材：**GA 組（程式碼端）**與 **GB 組（規格端）**兩份獨立 draft（2026-09-22）。⚠️ **兩組皆為單組結論，⛔ 未互相複驗**；⚠️ ~~**本頁與前兩頁不同 —— 沒有任何一條經總管親自複驗**（⛔ 無 `F-0`／`W-0` 的對應物，見 `Q-0`）。~~ **← 2026-09-23 事實更正，⛔ 不是漏刪**（執行 **KC 落地組**）：本頁自本日起**有**經總管親自複驗的承重條目（見 `Q-0-1`），該句的「0 條」前提已不成立；⚠️ **但只有標 ★ 者升格，本節其餘條目仍是單組結論、打折照舊** —— 兩邊理由並陳見 `Q-0`。對照表本體＝`docs/v2/spec/DATA_MAP_INSPECT.md`（22 block）。
...
### Q-0. ~~⚠️ 本頁「總管親自複驗」＝ **0 條**（據實登記，⛔ 不是漏寫）~~ → **現行：本頁已有總管親自複驗的承重條目（2026-09-23 更正，⛔ 不是漏刪）**

- ~~找標的頁有 `F-0` 三條、憑什麼頁有 `W-0` 六條**經總管親自複驗**的承重發現；**🔬 查一檔頁一條都沒有** ⇒ **本節全部條目一律照原組的單組標示打折**，⛔ 不得因為它被寫進本檔就升格為事實。~~
- ⚠️ **2026-09-23 事實更正，⛔ 不是漏刪** —— **執行**：**KC 落地組**；**覆核來源**：**KA（實作端）＋ KB（規格端）＋ 總管親自複驗**（三方交叉）。**被推翻的是「0 條」這個前提，⛔ 不是那條打折紀律。**
  - **舊句在寫下的當天逐字為真，⛔ 不是寫錯**：2026-09-22 彙整當下，本頁確實**一條都沒有**經總管複驗 —— 該句據實登記了那個事實（會漂移的量測值一律標日期，`CLAUDE.md §8.2.A.0` 規則 4）。
  - **新事實為何勝出**：2026-09-23 就 `inspect.stock.health` 跑了一輪**三方交叉**，**總管親自複驗了其中一批實作端事實** ⇒ 本頁自本日起**有**經總管複驗的承重條目，逐條見 **`Q-0-1`**（段內標 **★** 者即總管複驗項）。
  - ⚠️ **⛔ 不得把本段讀成「本節全部條目都升格了」** —— 更正的只有「**0 條**」這個計數。**`Q-1`~`Q-6` 全部條目一字未改，仍是 GA／GB 的單組結論，打折照舊**（`Q-6` 誠實揭露全部仍然有效）。
  - ⚠️ **本節自本日起有三種可信度，引用時必須寫明是哪一種，⛔ 不得一律寫成「已查證」**：**★ ＝ 總管已親自複驗**／**〔KB 規格端〕＝ KB 單組、總管⛔ 未複驗**／**〔總管單組〕＝ 總管一人查、⛔ 未經第二組複驗**。
...
- **`Q-0-1`｜🔴 ★ 承重裁決：`inspect.stock.health` ＝ 財報體檢分，⛔ 不是六因子技術分（三方交叉：**KA 實作端 ＋ KB 規格端 ＋ 總管親自複驗**，2026-09-23）。** 本條**裁決** `Q-5-1` 的「兩組指到兩套不同的算分引擎，⛔ 並陳不裁決」。四段如下 ——
  - **(1) ★ 這個 block 實際跑的是財報體檢那一套（實作端；標 ★ 者＝總管親自複驗）。** 鏈路逐段：**L1 `src/data/core/financial_statements_fetcher.py::fetch_financial_statements`**（價／KD 另走 **L1 `src/data/core/data_loader.py::StockDataLoader.get_combined_data`**）→ **L3 `src/services/financial_health_engine.py::analyze_financial_health`**（⭐ **只吐 6 個 `*_module`、⛔ 本身不產分數**）→ **L3 同檔 `::no_ai_overall_verdict`**（⭐ **分數真正在這裡算**）→ **L3 `src/services/dividend_station_service.py::fetch_metrics`**（裝成 `mj_score_pct`／`mj_grade`／`mj_headline`／`mj_fail_items`）→ **UI `src/ui/views/page_inspect.py::_fetch_metrics` → `::load_stock_readout` → `::build_health_card`**，畫面字串 `"{score} / 100 · {grade} 級"`。★ **總管逐字複驗**：`page_inspect.py` 內逐字 `score_pct=_num(_m.get("mj_score_pct"))`；★ `::build_health_card` 內 `Card(key="inspect.stock.health", label="健康度")` 是**字面量**（⛔ 非 f-string），**全檔 2 處**。**算式**：`_pts ∈ {+2,+1,−1,−2}`／`max_pts = len(valid)*2`／`score_pct = round(total_pts/max_pts*100)` ⇒ **值域 `[−100, 100]`、⭐ 可為負**；**缺表／無有效項 → `score_pct: None`**。**分級 `A+／A／B+／B／C／F／N/A`，主要由 `fail_items` 個數決定**（`is_dying or ≥4→F`／`≥2→C`／`==1→B+`／`is_cashcow→A+`／`score_pct≥70→A`／else `B`）⇒ **⛔ 不是分數帶切出來的**（與 `docs/v2/spec/S1-2_METRIC_SSOT.md §5` 的 `D-9` 逐字「由 fail_items 數決定，非分數帶」**同向**〔KB〕）。★ **門檻** `shared/financial_health_thresholds.py` 的 `^FH_` 常數**實測 22 個**（⛔ **非 19** —— 對照表該列已就地更正）。
    - ⚠️ **兩處與對照表既有記載不一致，⛔ 本輪並陳、未自行統一**：(a) **價的來源** —— `DATA_MAP_INSPECT.md` 該列「資料來源」欄現寫 `src/data/stock/app_stock_fetchers.py::fetch_price_data`，本段（KA／總管）寫的是 `data_loader.py::StockDataLoader.get_combined_data` ⇒ **⛔ 本輪未查證哪一個是現行路徑、也可能兩者皆有 ⇒ ⚠️ 待確認**（該欄**原文未動**）；(b) **值域** —— 該列末寫「分數值域 `[0,100]`（`CLAUDE.md §3.2`）」，本段為 `[−100,100]` ⇒ **⛔ 本輪未查證 `§3.2` 那一列指的是哪一套健康評分 ⇒ ⚠️ 待確認**（該句**原文未動**，僅於「計算」欄開頭加裁決指標）。
  - **(2) ★ 六因子 `calc_health_score` 真實存在、有 caller，但⛔ 與本 block 無接線。** ★ 真實住址 **`src/compute/scoring/scoring_helpers.py::calc_health_score`**；★ 簽章 `(df, rsi, ibs, vr, k_val, d_val, bb)` ⇒ **參數全是技術指標、⛔ 零財報欄位**；六因子＝**趨勢 30／RSI 20／量比 15／KD 15／IBS 10／布林 10 ＝ 100**。★ **總管實測全 caller 共 4 個**：`src/ui/tabs/tab_stock.py`／`src/ui/tabs/stock_grp_sections/section_batch_fetcher.py`／`src/services/health_history_service.py`／`scripts/update_health_history.py` ⇒ **餵的是舊 `src/ui/tabs/` 個股頁那一套**。★ **`src/ui/views/`（IA v2 新五頁）⛔ 無任何一處呼叫它** —— `src/ui/views/page_why.py` 的命中**全是說明文字、⛔ 不是呼叫**。
    - ⇒ 🔴 **兩組⛔ 不是誰對誰錯，是各自答了兩個不同的指標。** GA 答的是「**這張卡實際顯示什麼**」（財報體檢分），GB 答的是「**規格裡那個叫「健康評分」的指標怎麼算**」（六因子）—— **兩邊各自都成立，只是⛔ 不是同一個東西**。⚠️ 這與 `docs/v2/spec/S2-UI_SPEC.md §9.3` 的 **`D-6`** 逐字「⚠️「健康度」在本頁與「🔬 查一檔」是**兩套不同刻度，不可互比**」**同向** ⇒ **規格端早已承認兩套刻度並存**〔KB 規格端，總管⛔ 未複驗〕。
  - **(3) 🔴 規格端從未指定過本 block 的算分引擎 ⇒ 這是「規格洞」，⛔ 不是單純的「掛錯」**〔KB 規格端，總管⛔ 未複驗〕。`docs/v2/spec/UI_PAGE_INSPECT.md` 全檔 `六因子`／`財報體檢`／`健康評分`／`calc_health_score`／`health_thresholds` **0 命中**，**只指名 L5 卡片建構函式 `build_health_card`** ⇒ **規格從頭到尾沒說這張卡該用哪一套算分**。🔴 **而線框 `docs/v2/prototype/wf_page_inspect.js` 在同一個 block 內部就自相矛盾**：`missing` 逐字「**六因子**的輸入這一輪一項都沒拿到」／`partial` 逐字「六因子中 5 項計入；趨勢因子無 MA 資料，未計入」／`degraded` 逐字「78 分（趨勢因子無 MA，不計入）」＝ **指向六因子**；但**同一個 block** 的 `live` 時點逐字 **`2026-Q2 財報`** ＝ **指向季報那一套**。**MA／趨勢是日線技術指標，⛔ 不可能有「2026-Q2 財報」這個時點。** 而 `UI_PAGE_INSPECT.md` 自陳「**基準**：線框 `wf_page_inspect.js` ＝ 版面來源」⇒ **矛盾被原樣繼承，規格端至今⛔ 未裁決**。⚠️ **`78 / 100` 與 `B 級` 在兩套等第裡都存在** ⇒ ⛔ **不得拿 `B 級` 當任一方的佐證**〔KB 自陳〕。⚠️ `docs/v2/spec/INDICATOR_SPEC.md §4` 把兩者列為**兩列**：「個股健康評分｜6 因子／100」vs「財報體檢｜11 項 `Status` → `_pts ∈ {+2,+1,−1,−2}`｜11 項／22（**負分制**）」⇒ **規格端自己分得清兩套，只是⛔ 沒說這張卡用哪一套。**
  - **(4) 編號掛載：`D-35` 掛錯、`D-29`(b) 射程偏離、`D-28`／`D-29`(a) 與本卡實作對不上**〔KB 規格端，總管⛔ 未複驗〕。`D-28`／`D-29`／`D-35` 出自 **`docs/v2/spec/DECISION_RULES.md §8`**（該節標頭逐字「接續 `INDICATOR_SPEC §5` 的 `D-27`」），三條的「檔案:符號」欄**全部釘在 `src/compute/scoring/scoring_helpers.py::calc_health_score`** ⇒ **100% 屬六因子那一套** ⇒ **既然本 block 走財報體檢，這三條掛在這裡就與實作對不上**。🔴 **`D-35` 是掛錯，且此結論⛔ 不依賴「本 block 跑哪一套」**：`D-35` 規範的是**跨股票排名**，其檔案欄逐字列的符號**全部在「🏆 個股組合」批次管線**（產生端唯一 `run_batch_fetch`）；而 `inspect.stock.health` 是**單檔診斷葉 `l1`**（`UI_PAGE_INSPECT §⑤`），**結構上不存在跨股排名** ⇒ **即使本 block 真走六因子，`D-35` 也不該掛這裡**。**`D-29`(b) 同理射程偏離**（**(a) 才是 `calc_health_score` 本體**）。
    - ⚠️ **財報體檢（`FH_*`）在規格端查無對應 `D-*` 編號**〔KB〕：`INDICATOR_SPEC §4` 的「財報體檢」那一列**沒掛任何 `D-*`**；最接近的 `D-20` 講的是 **rescale／gate**、⛔ **不是 `FH_*` 下沉**。⇒ 🔴 **⛔ 不得自行改掛任何編號 —— 沒有可改掛的對象。** 本輪**只登記、⛔ 不動編號**：對照表該列的 `D-01`／`D-28`／`D-29`／`D-35` **原文一字未刪**，僅就地加註。
    - **撞號檢查（⛔ 不得補零統一）**〔KB〕：`S2-UI_SPEC §9.3` 是**獨立一套** `D-1`…`D-7`（未補零、**止於 `D-7`**）⇒ `D-28`／`D-29`／`D-35` **沒有同名對手**；而 `D-28` 與 `D-01` 是**同一套**（`DECISION_RULES §8` 明文接續 `D-27`，只是換了補零風格）⇒ **⛔ 不得補零統一**（同 `F-0-1` 與本檔索引 `DATA_MAP_ALL_PAGES.md §2` 共通前提）。
    - ⚠️ **本條的三種可信度，引用時必須照抄**：標 **★** 者 ＝ **KA 查、總管親自複驗**；**(3)(4) 全段 ＝ KB 單組結論，總管⛔ 未複驗**。⚠️ **KA 自陳**：產生器 `docs/v2/prototype/gen_today_v2.py` 對本 block **只 demo `error` 一態**（`value=None`）、**原型裡⛔ 沒有 live 數值樣本**；且**值域 `[−100,100]` 是由配點表推得、⛔ 非 runtime 實測**。⚠️ **KB 自陳查不到**：本 block 實際跑哪一套（KB ⛔ 禁讀 `src/`）／`2026-Q2 財報` 是筆誤或忠實／`FH_*` 個數規格端無從複驗／**`S1-2_METRIC_SSOT.md` 完全沒有「個股六因子」章節**（`§2.1` 是**總經**健康評分、`§2.3` 是財報體檢）⇒ **六因子在指標 SSOT 附錄無落點，⚠️ 可能是另一個獨立規格洞，本輪⛔ 未展開**。
- **`Q-0-2`｜★ 已排除的一個誤判：`no_ai_overall_verdict` 的 `else 50` 是「死路」，⛔ 不是 `CLAUDE.md §1` 造假（總管排除，2026-09-23）。** `src/services/financial_health_engine.py::no_ai_overall_verdict` 內有一句 `score_pct = … if max_pts > 0 else 50` —— **看起來**正是 §1 點名的「**自己估一個合理值**」（拿零筆資料發一張 **50 分及格證書**）。★ **總管實測：那個 50 出不去** —— `if not valid:` 守衛**先行** `return {…"score_pct": None…}`，⇒ `max_pts == 0` 在 production 路徑**到不了那一行**；**檔內註解自陳這正是他們修掉的東西**。⇒ 🔴 **⛔ 不是缺陷、⛔ 不得再被提成 bug。**
  - **為什麼要登記一個「不是 bug」的東西**：它**長得很像違憲**，⛔ 不登記的話**下一個人（含未來的 AI）會再提一次、再查一次** —— 登記的目的是**把「已經查過、結論是死碼」這件事留下來**，省掉重複稽核。⚠️ 這與 `CLAUDE.md §-2` 的實證**同構但方向相反**：那一次是「**宣稱修好了、其實那段是死碼**」，這一次是「**看起來是 bug、其實它到不了**」—— **兩種都只有真的追過控制流才知道。**
  - ⚠️ **效力邊界**：★ 這是**總管親自複驗**的結論，但它**只涵蓋 `max_pts == 0` 這一條路徑**，⛔ **不是**對 `no_ai_overall_verdict` 全函式的合憲性背書。
- **`Q-0-3`｜⚠️ 待查（總管單組、⛔ 未經第二組複驗）：📖 憑什麼頁解釋的健康分，與 🔬 查一檔頁顯示的健康分可能對不上。** `src/ui/views/page_why.py` **以說明文字描述** `calc_health_score` 的**六因子權重**（⛔ **非函式呼叫** —— 見 `Q-0-1`(2)：`src/ui/views/` 五頁 0 呼叫）⇒ **📖 憑什麼頁在解釋「技術面健康分」，而 🔬 查一檔頁顯示的是「財報體檢分」**。⚠️ 此與 `S2-UI_SPEC §9.3` 的 `D-6`「兩套不同刻度，⛔ 不可互比」**同向**，但**兩頁是否確實對不上，本輪⛔ 未展開** ⇒ **登記為待查，⛔ 不是結論。**
  - ⚠️ **可信度：總管單組，⛔ 未經第二組複驗**（`CLAUDE.md §-2` 規則 6）⇒ **只能當待驗事項**，⛔ 不得作為後續動作的前提、⛔ 不得寫進 commit message／PR 描述當成已完成的事實。
  - ⛔ **本條⛔ 不構成動工授權**（`CLAUDE.md §-1`：沒有 user 指派、沒有實際 bug 觸發 → **停手等指令**）；⚠️ 若日後有任務碰到這兩頁，請先**派獨立一組**查證（`§-2` 規則 4：⛔ 不可由想動手的那一組自己查）。
...
- **`Q-5-1`｜🔴 `inspect.stock.health` 的算分引擎，兩組指到兩套不同的東西（本頁最承重的一條）。** 〔**GA 程式碼端**〕鏈路是 **L3 `src/services/financial_health_engine.py::analyze_financial_health`**（no-AI 路徑）算綜合分與等第，門檻走 **L0 `shared/financial_health_thresholds.py`（`FH_*` 19 個）**，卡面欄位為 `mj_score_pct`／`mj_grade`／`mj_headline`／`mj_fail_items`；〔**GB 規格端**〕上游算式是 **L2 `src/compute/scoring/scoring_helpers.py::calc_health_score`** 的**六因子配分**（`DECISION_RULES §2 R2-1`：趨勢 30／RSI 20／量比 15／KD 15／IBS 10／布林 10；總分 `round(Σ得分ᵢ ÷ Σ滿分ᵢ × 100, 1)`），等第走 `shared/health_thresholds.py::HEALTH_GRADE_A_MIN(80)`／`B_MIN(50)`。⚠️ **GB 自陳⛔ 未讀任何 `.py`**，並自己寫「與 GA 交叉比對時**以 GA 的實測為準**」；⚠️ 但 GB 掛在該 block 上的 `D-28`／`D-29`／`D-35` 全部是**六因子那一套**的應然差異 ⇒ **若 GA 是對的，那幾個編號就掛錯 block 了**。**⛔ 本檔不裁決 —— 這一條建議優先派獨立一組覆核（`CLAUDE.md §-2` 規則 4）。**
  - 📌 **2026-09-23 已由 `Q-0-1` 裁決**（三方交叉：**KA 實作端 ＋ KB 規格端 ＋ 總管親自複驗**）—— **上面那條原文一字未刪、⛔ 不是漏刪**：它**忠實記錄了當天兩組的並陳狀態**，是裁決的來歷。**裁決結論一句話**：**本 block ＝ 財報體檢分**（`src/services/financial_health_engine.py::no_ai_overall_verdict`），六因子 `calc_health_score` **真實存在但與本 block ⛔ 無接線** ⇒ **兩組⛔ 不是誰對誰錯，是各自答了兩個不同的指標**；GB 掛的 `D-28`／`D-29`／`D-35` **確實掛錯 block**（其中 `D-35` **連六因子都不該掛** —— 射程＝跨股排名），⚠️ **但財報體檢規格端查無對應編號 ⇒ ⛔ 不改掛、只登記**。
...
- ⚠️ **本撰寫組（收斂組）自己的限制**：本輪**只做收斂、⛔ 未重新調查** —— **⛔ 未讀 `src/` 任何一個檔、⛔ 未跑產生器、⛔ 未跑任何測試、⛔ 未打開畫面**；⛔ **未替任何欄位發明中文標籤、⛔ 未發明任何資料源或算式、⛔ 未替任何「查不到」補一個合理猜測**；兩組打架一律**並陳 ＋ 標明來自哪一組**（`Q-5`），**⛔ 未自行統一**。⚠️ ~~**`docs/v2/spec/DATA_MAP_ALL_PAGES.md` 索引內「🔬 查一檔」那一格仍寫「⚠️ 尚未製作」，本輪⛔ 未動**（未獲授權）。~~
  - ⚠️ **2026-09-23 事實更正，⛔ 不是漏刪**；執行 **GD 更正組**。**現行**：索引內「🔬 查一檔」那一格已由 **KC 落地組**於 commit `cf435db` 填上實際行數與 block 數（**58 行／22 block**，量測日 **2026-09-23**）；「💼 我的持股」那一格亦已由 **HC 撰寫組**於 commit `f89093d` 填上（**53 行／23 block**，同日）⇒ `docs/v2/spec/DATA_MAP_ALL_PAGES.md` 內**⛔ 已無任何一格寫「尚未製作」**。⚠️ 行數是會漂移的量測值（`CLAUDE.md §8.2.A.0` 規則 4）：**SSOT ＝ 索引表本身**，需要時現場 `wc -l`，⛔ 不要引用本行當往後的事實。
  - **兩邊理由並陳 —— ⭐ 本條住在 `Q-6`「誠實揭露」節，故必須先講清楚哪一半⛔ 沒有失效**：〔**仍然成立、⛔ 不是失誤的那一半**〕「**本輪未獲授權 ⇒ ⛔ 未動**」這個**當時的處置本身完全正確** —— 那是 `CLAUDE.md §-1` 動工閘門（沒有 user 指派／沒有實際 bug 觸發就不要碰）的正確執行，**一字未減，仍然完全有效**；本條記錄它，正是為了讓讀者知道「那一格當時**為什麼**沒被順手改掉」。〔**被推翻的那一半**〕只有「那一格『**仍**』寫著尚未製作」這個**狀態描述** —— 它在本節彙整當下逐字為真，其後由**另外兩組在各自獲授權的批次裡**補上（`cf435db`／`f89093d`）⇒ **⛔ 不是本撰寫組漏做，更⛔ 不是當時該做而沒做。**
  - ⚠️ **⛔ 不得把本段讀成「`Q-6` 其他揭露也跟著失效」** —— 更正的只有上面那一句**狀態描述**。本節其餘各條**一字未改、全部仍然有效，打折照舊**（GA／GB 兩組皆單組結論且⛔ 未互相複驗、GA 有 AST 但⛔ 未實跑任何一頁、GB ⛔ 未讀任何 `.py`、兩份大檔未讀完、線框 `live`／`partial` 數字⛔ 非實測、本撰寫組⛔ 未重新調查）。
