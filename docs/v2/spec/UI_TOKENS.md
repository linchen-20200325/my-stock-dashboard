...
| `--sig-neutral` / `--sig-neutral-bg` | **`#8fa3b8`** / **`#26313d`**（原 ~~`#1f2a36`~~） | **待訂** / **待訂** | **資料不完整（N／M 部分計入）** | ◧「N／M 計入」 | **新訂**（客戶 2026-09-21 裁示；`--sig-neutral-bg` 同日自 `#1f2a36` 改採總管建議值，`--sig-neutral` 未動，見 A-2.1；線框無此格） |
...
- 🔴 **仍未解決（前景，本輪未動）**：`--sig-neutral` `#8fa3b8` vs `--sig-grey` `#8b98a4` ＝ **ΔE2000 4.85**，**遠低於本檔 A-2 自訂的 ≥15 可辨門檻**。#9 在 §2 徽章表中與 #2／#3／#5／#7／#8 五列同框，那五列前景全是 `--sig-grey` ⇒ **新前景與灰系前景幾乎同色**，靠 A-2「⛔ 一律配圖示＋文字」擋，⛔ 不得讓兩者同框而只靠顏色分辨。
...
- ~~`--sig-neutral-bg` `#1f2a36` vs `--sig-grey-bg` `#1b232b` ＝ **ΔE2000 2.98**；vs `--panel-2` `#1b242f` ＝ **ΔE2000 1.98**（即新底色與它要取代的舊底色本身也近乎同色，與上條「只改善 0.09」同一件事的另一個量法）。~~
  → **舊值 `#1f2a36` 的量測，2026-09-21 客戶改採 `#26313d` 後不再適用。**
  ⚠️ **但新值 `#26313d` 對這兩個背景（`--sig-grey-bg`／`--panel-2`）本輪未重算** —— ⛔ 不得讀作已解決，light 值補量時一併補。
