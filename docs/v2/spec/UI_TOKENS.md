...
| `--sig-neutral` / `--sig-neutral-bg` | **`#8fa3b8`** / **`#1f2a36`** | **待訂** / **待訂** | **資料不完整（N／M 部分計入）** | ◧「N／M 計入」 | **新訂**（客戶 2026-09-21 指定值；線框無此格） |
...
#### A-2.1 `--sig-neutral` / `--sig-neutral-bg` 的量測與未達成事項（2026-09-21 新增）

**用途**：僅供 `UI_COMPONENTS` §2 狀態徽章 **#9「資料不完整」**（底色＋文字色）。⛔ 框色不在本對範圍內，#9 框色維持 `--sig-blue`。
**light 值＝待訂**：客戶 2026-09-21 只指定 dark 兩碼，⛔ 本檔不得自行發明 light 值；light 模式落地前必須回頭補量。

- ✅ **文字對比達標**：`#8fa3b8` on `#1f2a36` ＝ **5.61:1**，高於 WCAG AA 小字門檻 4.5:1（徽章字級 11px／10.5px，⛔ 不適用大字 3:1 寬鬆標準），亦高於客戶要求的 3:1。
- ⚠️ **但「與 `--sig-blue-bg` 明顯不同」這項未達成**：`#1f2a36` vs `--sig-blue-bg` `#141f2b` ＝ **對比 1.14:1、ΔE2000 3.45**；vs 卡面 `--panel` `#151d26` ＝ **1.17:1、ΔE2000 4.26**。
  對照：**原本 #9 用的 `--panel-2` 對卡面是 1.08:1** ⇒ 新值只改善 0.09，**「中性底在卡上看不見」這個原始問題並未解決**。
- 📌 **總管建議值（待客戶裁示，本輪未採用）**：**`#26313d`** —— 文字對比 **5.10:1**（仍高於 AA）、vs `--sig-blue-bg` **ΔE2000 5.67**、vs `--panel` **ΔE2000 6.36**，兩者都進入「一眼可辨」區間。
  （另評估過 `#2b3947`：ΔE 8.18 更清楚，但文字對比掉到 **4.55**，只比 AA 底線高 0.05，餘裕太薄，**不建議**。）
- ⚠️ **本輪照客戶指定的 `#1f2a36` 落地**；顏色屬客戶決定，總管只負責量測與揭露。

⚠️ **查證分級**：上列五項為**總管實算**，**撰寫組 WE2 已獨立複算，逐項數字相符**（WCAG 相對亮度公式＋ΔE2000，非目測）。
⚠️ **以下為 WE2 自算、總管交辦範圍外的相鄰風險（單組未複驗，⛔ 不得當既定前提）**：
- `--sig-neutral` `#8fa3b8` vs `--sig-grey` `#8b98a4` ＝ **ΔE2000 4.85**，**遠低於本檔 A-2 自訂的 ≥15 可辨門檻**。#9 在 §2 徽章表中與 #2／#3／#5／#7／#8 五列同框，那五列前景全是 `--sig-grey` ⇒ **新前景與灰系前景幾乎同色**，靠 A-2「⛔ 一律配圖示＋文字」擋，⛔ 不得讓兩者同框而只靠顏色分辨。
...
- `--sig-neutral` 對三背景的文字對比：`--panel` **6.55:1**／`--paper` **7.14:1**／`--panel-2` **6.04:1**（比照 A-1 `--ink-3` 三背景定錨慣例，三格皆 ≥ 4.5 PASS）。
