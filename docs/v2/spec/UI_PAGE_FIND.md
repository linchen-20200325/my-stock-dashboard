...
**(a) 應然（畫面怎麼畫）** —— `本益比` 欄三分支，⛔ 不得同形（**重試有用性相反**）：
...
⇒ **在 `D-08` 修完之前，第四層只能照線框 `live` 現況那段誠實寫「本欄目前分不出這兩種」**，⛔ 不得先畫成分得開的樣子。
...
|---|---|---|---|
| **結果表欄位** | 6 欄固定：`代號｜名稱｜評分｜本益比｜殖利率｜缺貨`，值為原始值（`18.20`） | `代碼｜名稱｜綜合分｜〈估值分／EPS分／缺貨分／RS分／跨季分〉`（依勾選動態增欄），因子值為**百分位 0–100、1 位小數**；**無殖利率欄** | **設計變更（線框待改），但本益比欄例外保留原始值。** 三條理由：(a) 因子欄是**勾選決定的動態欄**，線框的固定 6 欄模型描述不了；(b) 百分位是 L3 排序用的同一把尺，改回原始值會讓「綜合分」與欄值兩套尺並存（違 §2.1 SSOT）；(c) **但** ③④ 的三語彙同框需要至少一個**原始值欄**當載體 ⇒ **`本益比` 一欄照線框保留為原始值**。**殖利率欄：不加** —— 來源端「沒配息」與「還沒公告」本身就分不開，加了只會多一欄畫不出三語彙的欄，**線框待刪該欄**。 |
...
**WH 本組判定（⛔ 不是量測、⛔ 不是客戶裁示）**＝ ④ 不採用 `⚠ 非正值`（併入 #8）、⑥ 結果表欄位的分類與「本益比欄保留原始值／殖利率欄不加」、`n5` 取 `t2` 密度、葉2 與 CSV 鈕對映到 §3 次級（說明）。**四項皆須客戶或第二組覆核。**
