...
- **n0 葉外**：`inspect.statusbar`／`chrome.asof`／`chrome.footer` 在 `page_inspect.py` **0 命中（WM 實測）** ⇒ 失敗時無可顯示；⛔ 不得用判決卡的紅態冒充一條不存在的狀態列。⚠️ `chrome.footer`（頁尾免責，`D-1`「**每一頁**」）**現況本頁沒有** —— 登記為實作待修。
...
**WM 本組判定（⛔ 不是量測、⛔ 不是客戶裁示，全部須覆核）**＝(1) ③① 建議「先畫兩張 unwired 卡」為現行落地態；(2) ③② 現行誠實文案擬稿與「⛔ 畫面不得寫『本系統採加權移動平均法』」；(3) ② 末段「`profit` 的 3/5 與 `batch` 的 N/M 母體不同類、須在 tooltip 寫明母體」；(4) ④B 末項「整列可點未落地」的發現與其**待判分類**；(5) 葉2 CTA 對映到 §3「主 CTA 同款」（⛔ 更正派工單所述的「次級」）；(6) `chrome.footer` 缺頁尾免責登記為實作待修。
