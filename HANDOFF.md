...
> **量測基準**：`origin/main` `07f8800`（PR #695）＋ PR #696 待合併（僅測試）。「誰決定」＝客戶 or 總管；標 ⚠️ 者為**總管判讀**，客戶一句話可推翻。
> ⚠️ 本清單由**單組**彙整（§-2 規則 6），⛔ 不得當成「repo 已無其他待辦」的全稱前提。

**A. 本輪進行中（客戶 2026-09-26 核准）**

| # | 項目 | 現狀 | 卡在哪 | 下一步 | 誰決定 |
|---|---|---|---|---|---|
| A1 | 燈卡「▸ 詳細」鍵盤／螢幕朗讀 a11y（來源 §6.4） | 動工中；只改共用元件 **markup** | — | 交付 PR（可聚焦＋`aria-expanded`＋焦點樣式） | 客戶已核准 |
| A2 | 「變化方向」列補 4 盞需 L3 的燈：`health`（健康分數）、`jingqi`、`ndc_signal`（國發會景氣燈號）、`tw_export`（台灣出口） | 動工中 | 需 L3 提供序列 | 交付 PR | 客戶已核准 |

**B. 等客戶（規格／文案／新源／授權）**

| # | 項目 | 現狀 | 卡在哪 | 下一步 | 誰決定 |
|---|---|---|---|---|---|
| B1 | 方向列 `us10y`（美債 10 年） | 「無資料」 | 燈只用單點 FRED；`^TNX` 是不同源 | 客戶指定可接受的新源 | 客戶 |
| B2 | 方向列 `dxy`（美元指數） | 「無資料」 | 本地無歷史可量平盤帶；備援 DX-Y.NYB→DX=F→UUP 尺度不同 | 同上 | 客戶 |
| B3 | 方向列其餘無資料：`fut_net`（僅存 14 天）、`us_core_cpi`（只有前值、無日期）、`news_systemic`（無數值序列、判準未定義）、`foreign_net`（燈未接線；FinMind 法人單位未確認 §4.1） | 「無資料」 | 各列括號 | `fut_net` 等累積；`us_core_cpi` 需帶日期的源；`news_systemic` 需判準規格；`foreign_net` 先查證單位 | ⚠️ `fut_net`／`foreign_net`＝總管；`us_core_cpi`／`news_systemic`＝客戶 |
...
| B6 | 標記矛盾：#7「缺漏·可重跑」vs 文字「重按也一樣」／「有效的結果」—— hold／inspect／why 其餘卡**合一條**（客戶：不另開單）。含已修靜默失敗卡（`find.screen_result`／`hold.take_profit`／`hold.switch`／`hold.deep.dividend_cash`）尚未登記進 `V2_VALID_EMPTY_SPEC`（main 上只登記 `hold.portfolio_count`） | 未修 | 與持股頁同一根因；改 builder／文案需授權 | 客戶授權後一次修 | 客戶 |
...
| # | 項目 | 理由 | 誰決定 |
|---|---|---|---|
| C1 | `adl`（騰落比）方向列 **不做**，維持「無資料」 | `ad_ratio` 為單日估計，幾乎無自相關（`twii_ohlcv.parquet` lag-1 −0.028、lag-20 0.016）⇒ 20 日比較會誤導 | 總管（已記錄供客戶知悉） |
