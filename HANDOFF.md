...
**計數**（清點日 2026-09-26；編修組清點，**2026-09-26 二組複核更正**：B7a 待辦→已完、新增 DOC2 待辦；其餘列數與狀態二組逐列重算一致；commit 前依更新觸發點 ③ 將 (d)「清點交接本」稽核中→已完）；⚠️ 單組 —— 現行數字為 2026-09-26 ~~三更新~~ 四更新逐列重數（方法見表下）

| 類別 | 已完 | 進行中 | 待辦 | 卡住 | 合計 |
|---|---|---|---|---|---|
| (a) 五頁卡化 | 11 | 0 | 0 | 1 | 12 |
| (b) 燈號「變化方向」（16 盞） | 7（真方向 5＋定案不做 2） | 0 | 0 | 9 | 16 |
| (c) 其他 | ~~7~~ ~~8~~ ~~9~~ ~~10~~ ~~11~~ **13**（含定案不做 2；+S5-H；+S5-W；+S5-F；+Q1；+C6 查證） | ~~3~~ ~~4~~ ~~3~~ ~~3~~ **5**（Q2／Q5／B9 ⑥⑦⑧） | ~~10~~ ~~8~~ ~~13~~ ~~17~~ **13**（含 Q1-r2～r5、Q5-r1、B9 ⑨） | ~~12~~ ~~13~~ ~~15~~ **16**（含 Q1-r1、Q5-r2、C6-del） | ~~32~~ ~~33~~ ~~34~~ ~~39~~ ~~46~~ **47** |
| (d) 派工追蹤 | ~~1~~ ~~2~~ ~~3~~ ~~4~~ **5**（+S5-H；+S5-W；+S5-F；+Q1） | ~~0~~ ~~3~~ ~~4~~ ~~3~~ ~~3~~ **3**（Q5 搬主線解衝突中；Q2 實作中；B9 ⑥～⑧ 實作中） | 0 | 0 | ~~1~~ ~~4~~ ~~6~~ ~~7~~ **8** |
| **合計** | ~~25~~ ~~27~~ ~~29~~ ~~31~~ ~~33~~ **36** | ~~**3**~~ ~~6~~ ~~8~~ ~~6~~ ~~6~~ **8** | ~~10~~ ~~8~~ ~~13~~ ~~17~~ **13** | ~~22~~ ~~23~~ ~~25~~ **26** | ~~61~~ ~~62~~ ~~65~~ ~~68~~ ~~73~~ ~~81~~ **83** |
...
| 已派工 Q5 查一檔各卡~~，等回報~~ | **進行中（修正完成 → 搬主線解衝突中 → 待 QA 複驗）**（~~派工中~~ → 稽核中 → 2026-09-26 獨立 QA 回報 **1 項阻擋**〔B1〕→ ~~發回實作組修正中~~ → 2026-09-26 實作組回報已修好 B1 ＋ 3 個測試缺口〔⚠️ 實作組自述、**未經 QA 複驗**〕→ 搬主線解衝突中；未 merge） | —（待合併） | 📌 **2026-09-26 四更新**：工作區 `/home/user/msd-q5`（基底 `origin/main` `2810064`），正在解 `src/services/dividend_station_service.py` 與 #699 的衝突 → 送**原 QA 組**複驗 → PR → CI 綠 → merge → 移「已完成」附 hash。「2 處註解言過其實」本次回報未提及是否已修 ⇒ ⚠️ 待確認（QA 複驗時一併看）。~~修掉 B1 ＋ 3 個測試缺口 ＋ 2 處註解言過其實 → 重跑獨立 QA → PR → CI 綠 → merge → 移「已完成」附 hash。~~細節見 (c) c-1 Q5 列；非阻擋新待辦 **Q5-r1／Q5-r2**（c-2）。授權：批 5（見 c-1 表下 2026-09-26 總管判讀）。總管 |
...
| 已派工 Q2 `hold.alloc_split`／`hold.deep.core_satellite` 全部失敗仍落空態，等回報 | **派工中（實作中）**（2026-09-26） | —（待合併） | 回報 → 稽核中 → 獨立 QA → PR → CI 綠 → merge → 移「已完成」附 hash。工作區 `/home/user/msd-q2`（基底＝`origin/main` `3f4493c` ＋ Q1 本地 commit；編修組實測 HEAD `623e326`「local: Q1 base (not pushed)」，其父＝`3f4493c`）。📌 **2026-09-26 四更新（編修組實測）**：`623e326` 的 4 個 Q1 原始碼檔與 #702 merge 版**逐 blob 相同**，工作區未追蹤的 `tests/test_v2_silent_fail_b7_binding.py` 亦與 #702 版相同 ⇒ 搬到 `origin/main` `0769730` 時 Q1 部分不應再產生內容差異（⚠️ 衝突與否以實際 rebase 為準）。授權：批 5（見 c-1 表下 2026-09-26 總管判讀）。總管 |
| 已派工 B9 ⑥～⑧ S5-F QA 非阻擋殘留收尾（`page_find.py` 紅卡 where 句／「這輪失敗」用語／`RuntimeError()` 事實列），等回報 | **派工中（實作中）**（2026-09-26） | —（待合併） | 回報 → 稽核中 → 獨立 QA → PR → CI 綠 → merge → 移「已完成」附 hash。工作區 `/home/user/msd-b9`（基底 `origin/main` `2810064`；編修組實測當下未提交改動：`src/ui/views/page_find.py`、`tests/test_v2_silent_fail_b5_find.py`）。**B9 ⑨** 不在本次派工（仍待辦）。細節見 (c) c-2 B9 ⑥～⑧ 列。總管 |
...
| a2 🔍 找標的 `find`（4 張卡） | 已完 | #687 `83e0f47` | 殘留：**S5-F**（~~進行中；暫停 → 2026-09-26 已派工恢復 → 稽核中 → 稽核通過、PR #701 待合併~~ → **已完** #701 `2810064`，2026-09-26）、**B9 ⑥～⑨**（S5-F QA 殘留；前提「PR #701 merge 後」已成立；~~待辦~~ → 2026-09-26 **⑥～⑧ 進行中**〔已派工、實作中〕、**⑨ 待辦**）、**B6**（`find.screen_result`／`find.sector_flow`）。⚠️單組彙整 |
| a3 🔬 查一檔 `inspect`（14 張卡；批次表與下鑽格照舊） | 已完 | #691 `f38dbd4` | 殘留：**Q5** 各卡靜默失敗（~~待辦~~ → 2026-09-26 進行中，~~已派工~~ → 稽核中 → QA 回報 1 項阻擋 → ~~發回修正中~~ → 修正完成〔實作組自述〕→ 搬主線解衝突中 → 待 QA 複驗）、**Q5-r1**（待辦）／**Q5-r2**（卡住）、**B6**、**C5b** 未用 `UI_EMPTY` import。⚠️單組彙整 |
| a4 💼 我的持股 `hold`（23 張卡） | 已完 | #688 `66e4fe0` | 殘留：**S5-H**（~~進行中；暫停 → 2026-09-26 已派工恢復~~ → **已完** #699 `bd6ceec`，2026-09-26）、~~**Q1～Q4**（待辦）~~ →**Q1**（~~進行中，已派工 → 稽核中 → 2026-09-26 稽核通過、PR 準備中~~ → **已完** #702 `0769730`，2026-09-26）、~~**Q2～Q4**（待辦）~~ → **Q2**（2026-09-26 進行中，已派工、實作中）、**Q3～Q4**（待辦）、**Q1-r1**（卡住）／**Q1-r2～r5**（待辦）、**S4-T**（停利卡 why 句，待辦）、**B6**、**B9 ③**。⚠️單組彙整 |
...
| Q2 `hold.alloc_split`（`SPLIT_NO_VALUE_NOW`）／`hold.deep.core_satellite`「算不出比例」：全部失敗時仍落空態 | **進行中（派工中、實作中）**（~~待辦（佇列）~~；2026-09-26 已派工，等回報，見 (d)）⚠️單組（key↔常數對映） | —（待合併） | 工作區 `/home/user/msd-q2`（基底＝`origin/main` `3f4493c` ＋ Q1 本地 commit `623e326`）。回報 → 獨立 QA → PR → merge。授權同 Q1（批 5） |
...
| B9 ⑥ S5-F（PR #701）因子失敗紅卡的 where 句，對 PE／RS／趨勢因子仍指向「FinMind 額度／MOPS–Goodinfo 鏈」⇒ **誤導**（S5-F 獨立 QA 非阻擋發現，2026-09-26 登記）；趨勢因子較貼切的是既有 `SNAPSHOT_WAIT_WHERE` | **進行中（派工中、實作中）**（~~待辦~~ → 2026-09-26 已派工，工作區 `/home/user/msd-b9`，見 (d)；📌 收尾前提「PR #701 merge 後」**已成立**：#701 `2810064`）⚠️單組（一組 QA） | —（待合併） | 依因子改指既有 where 常數（趨勢 → `SNAPSHOT_WAIT_WHERE`）；PR #701 merge 後在 `src/ui/views/page_find.py` 收尾。總管 |
| B9 ⑦ S5-F 紅卡用語「這輪失敗」暗示一次性；但季快照缺是**持續性**的，重跑不會好（S5-F 獨立 QA 非阻擋發現，2026-09-26 登記） | **進行中（派工中、實作中）**（~~待辦~~ → 2026-09-26 已派工，工作區 `/home/user/msd-b9`，見 (d)；📌 收尾前提「PR #701 merge 後」**已成立**：#701 `2810064`）⚠️單組（一組 QA） | —（待合併） | 用語措辭。總管 |
| B9 ⑧ S5-F 紅卡事實列顯示「`RuntimeError()`」—— 對使用者無資訊量；可改沿用 L3 log 既有用語「跨季趨勢為空表」（S5-F 獨立 QA 非阻擋發現，2026-09-26 登記） | **進行中（派工中、實作中）**（~~待辦~~ → 2026-09-26 已派工，工作區 `/home/user/msd-b9`，見 (d)；📌 收尾前提「PR #701 merge 後」**已成立**：#701 `2810064`）⚠️單組（一組 QA） | —（待合併） | 措辭決定（沿用 L3 log 用語與否）。總管 |
| B9 ⑨ S5-F 規格缺口（S5-F 獨立 QA 非阻擋發現，2026-09-26 登記）：紅卡未提示「取消勾選這個因子」；未點名是**哪個市場**失敗；① 一個因子都沒勾（＝B6 子表 find ①）未處理 | 待辦（📌 2026-09-26 收尾前提「PR #701 merge 後」**已成立**：#701 `2810064`）⚠️單組（一組 QA） | —（待合併） | 規格補齊。總管。📌 2026-09-26：**不在** B9 ⑥～⑧ 的派工內；總管排程（見「給下一個 AI」第 1 點）未列入 ⇒ 位置未定（⚠️ 編修組照實轉述，未另排） |
...
| 拆分／壓力／VaR／現金 空態（`hold.alloc_split` `SPLIT_NO_VALUE_NOW`／`hold.deep.stress` `STRESS_EMPTY_NOW`／`hold.deep.var` `VAR_EMPTY_NOW`／`hold.deep.dividend_cash` `CASH_EMPTY_NOW`） | 空＝輸入缺漏，不是有效的零 | 拆分＝Q2（2026-09-26 已派工、實作中，—（待合併））；壓力／VaR／現金＝Q3（待辦） |
...
| 查一檔（inspect）各卡 | 抓取失敗時欄位為 `None`，與「真的沒有」同一條路徑；批次卡＝輸入空或格式錯 | ＝Q5（2026-09-26 進行中：QA 回報 1 項阻擋、~~發回修正中~~ → 修正完成〔實作組自述〕→ 搬主線解衝突中 → 待 QA 複驗；—（待合併）） |
...
📌 **2026-09-26 自檢計數器（四更新）**：自上次自檢以來 merge 了 #699 `bd6ceec`、#700 `3f4493c`、#701 `2810064`、#702 `0769730` 共 **4 項**（總管計）⇒ **距下次自檢 1 項**：**下一個 merge 後**即拿 `git log --merges --oneline origin/main` 對本表與主表，補漏。⚠️ 並陳：上行自檢紀錄的比對範圍已含 #699（「#685～#699」）⇒ 依紀錄為 3 項、距下次 2 項；**取嚴，照總管「下一個 merge 後自檢」執行**（提早自檢無害）。

### 給下一個 AI

> 固定格式（客戶 2026-09-26【交接表 · 維護時機】「交接段」）；**每次更新本節時同步改這五點**。以下據 2026-09-26 清點實況填寫（**第 1～2 點於 2026-09-26 派工 S5-H／W／F 時更新**；**2026-09-26 S5-H 已完 #699 `bd6ceec`／派工 Q1、Q5 時再更新第 1～5 點**；**2026-09-26 S5-W 已完 #700 `3f4493c`／S5-F PR #701 時再更新第 1～5 點**；**2026-09-26 S5-F 已完 #701 `2810064`／Q1 稽核通過／Q2 派工／Q5 發回修正時三更新第 1～5 點**；**2026-09-26 Q1 已完 #702 `0769730`／C6 查證完成／Q5 搬主線解衝突／B9 ⑥～⑧ 派工時四更新第 1～5 點**）；⚠️ 工作區內容為編修組讀 diff 所得，**單組、未 QA**。
...
2. **那項做到哪、下一步是什麼**（📌 2026-09-26 四改寫為 **Q5**；Q1 已結案 #702 `0769730`、S5-F 已結案 #701 `2810064`、S5-W 已結案 #700 `3f4493c`、S5-H 已結案 #699 `bd6ceec`，其描述保留於本點下方作歷史）：
