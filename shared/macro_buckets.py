...
class DangerSpec:
    ...
    # ── 2026-08-25:鑑別力(user 核准納入正式驗證流程)──────────────────────
    # `discriminative=False` = **這盞燈會亮,但它的判讀已經沒有意義**。
    #
    # 與 `wired=False` 的差別(兩者都是「別信這盞燈」,但成因與處置完全不同):
    #   wired=False           → 沒有值,燈**永遠不亮**(⬜)。處置:接線或別管它。
    #   discriminative=False  → **有值、燈也亮**,但門檻本身已失去判別力。
    #                           處置:別看它有沒有過線,改看它相對自身區間的方向。
    #
    # 為什麼一定要有這個旗標:一盞恆亮的燈等於沒有燈,而現行畫面上它
    # **跟一盞真正的紅燈長得一模一樣** —— 使用者沒有任何方式分辨。
    # 這個資訊原本只寫在本檔的開發者註解裡,使用者看不到。
    #
    # ⚠️ 本旗標**刻意不影響任何既有行為**:
    #   · 不改 `classify_danger` 的判燈結果(燈該紅還是紅)
    #   · 不改 readiness 分母(不同於 wired=False —— 那盞燈是真的沒值)
    #   它純粹是「加註」,讓畫面能誠實標示。要改判燈邏輯屬行為變更,需另案回測。
    discriminative: bool = True
    degraded_reason: str = ""  # discriminative=False 時**必填**(tests 有守衛)
                               # 寫給**一般使用者**看:為什麼不能信 + 該改看什麼
...
BUCKET_DANGER_SPECS: list[DangerSpec] = [
    ...
    DangerSpec("margin", "融資餘額", "chips", "億", "high_bad",
               ...
               #    是因為 `note` 目前是**唯一**會渲染到 production 五桶籌碼卡的欄位,
               #    刪掉會讓現行畫面直接少一段警語。等 production 卡片接上
               #    `degraded_reason` 後再收斂(屬畫面變更,§8.4 另案)。
               discriminative=False,
               degraded_reason=(
                   "這盞燈現在幾乎天天紅，但**不代表天天危險**。"
                   "它用的是固定金額當標準（超過 3,400 億就算過熱），"
                   "而這條線訂下來之後台股整體規模長大了不少，"
                   "現在的「正常水位」本來就在線上面。\n\n"
                   "**該怎麼看**：不要看它有沒有超過門檻，"
                   "改看它跟自己最近一兩年比，是變多還是變少 —— "
                   "融資快速增加而指數不漲，才是真正要留意的訊號。"
               )),
