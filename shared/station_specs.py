...
# ══════════════════════════════════════════════════════════════════
# 分組（`StationSpec.group`）的畫面名稱 —— SSOT（2026-09-09）
# ══════════════════════════════════════════════════════════════════
#
# 為什麼要有這張表：`group` 欄存的是**程式用的英文代碼**（health / timing /
# screen / stock）。畫面端若各自翻一次，就會出現「同一組在這一頁叫『四燈體檢』、
# 在那一頁叫『健檢』」的漂移；而不翻、直接把代碼原樣印出去
# （📖 憑什麼 那一頁的「分組」欄 2026-09-09 前的實況）等於把**內部代碼當成
# 使用者文案** —— 使用者看到 `screen` 不會知道那是 3-3-3 篩選。
# 兩種錯法都由這張表消除：**一組一個名字，只准定義在這裡。**
#
# 名稱由 user 2026-09-09 拍板（四個字串逐字照收，不得「順手優化」）。
# ⚠️ 刻意**不配 emoji**：總經那五桶的 emoji 是 `macro_buckets.BUCKET_META`
#    早就有的既有資產（讀它 = 讀 SSOT）；這四組**沒有**任何既有 emoji 可讀，
#    在這裡現編一個等於在 SSOT 裡放一個沒有出處的東西（§3.3）。
GROUP_HEALTH = "health"
GROUP_TIMING = "timing"
GROUP_SCREEN = "screen"
GROUP_STOCK = "stock"
...
#: 分組代碼 → 畫面中文名。
#: ⚠️ 消費端查不到 key 時**原樣顯示代碼**，不要編一個看起來合理的名字 ——
#:    查不到代表註冊表多了一組而這張表沒跟上，那是要修的 bug，不是要美化的畫面。
#:    守衛：`tests/test_station_specs.py::TestGroupTitles`（新增一組忘了命名 → CI 紅燈）。
STATION_GROUP_TITLES: dict[str, str] = {
    GROUP_HEALTH: "四燈體檢",
    GROUP_SCREEN: "3-3-3 篩選",
    GROUP_STOCK: "個股體質",
    GROUP_TIMING: "進場時機",
}
...
class StationSpec:
    ...
    #: 這盞燈**依規格就不產出等級** —— 不是「這輪算不出來」，是「還沒有判燈規則」。
    #: `False` 時 `no_level_reason` **必填**。
    #:
    #: 為什麼不改用 `MISS_NOT_APPLICABLE` 把它移出分母（2026-08-26 user 裁示時一併定案）：
    #: 那個原因的畫面文案是「這類持股不適用這盞燈（不是壞掉）」—— 對 KD 是**假話**。
    #: KD 對個股完全適用、K/D 也照抓照印，缺的是「幾點算 🟢、幾點算 🔴」這條規則本身。
    #: 為了讓分母好看而在畫面上寫一句假的，正是 §1 要擋的東西。故另立一個
    #: **結構性宣告**：這盞燈依規格就不出等級 → 不進「給得出判定」的分母；
    #: 但它照樣在看、照樣畫在燈格牆上、照樣有值可讀。
    #:
    #: 與另外兩個旗標的分工（三者互相獨立，不可互相替代）：
    #:   · `wired=False`          → 決策端刻意沒接，**燈根本不會亮**
    #:   · `discriminative=False` → 燈會亮、也有等級，只是門檻已失去判別力
    #:   · `emits_level=False`    → 燈亮著、資料也通，但**從來不出等級**
    #:
    #: 哪天真的替它接上判燈邏輯 → 把旗標翻回 `True`，分母自動長回來；
    #: 而「宣告會出等級卻始終不出等級」會被測試當場抓到（那才是 bug）。
...
STATION_SPECS: list[StationSpec] = [
    ...
    StationSpec(
        ...
        # 2026-08-26 user 裁示：把「結構上永遠不出等級」視同不適用、移出
        # 「給得出判定」的分母（個股分母 4 → 3）。
        # 為什麼非移不可：只要組合裡有任何一檔個股，這盞燈就讓那一列永遠不可能
        # 「每一盞都給得出判定」—— 巡航那句話因此**永遠印不出來**，變成死碼。
        # 一個永遠不會成立的 gate，跟沒有 gate 一樣沒有防護力，卻讓人以為有。
        # ⚠️ 移出的是**分母**，不是畫面：這盞燈照樣畫在燈格牆上、照樣可以點開看
        # K/D 值與鈍化／背離。它「在看」，只是不「給判定」。
        emits_level=False,
        # ⚠️ 這段話由明細面板**只看旗標、不看四態**印出來（面板端有對應註解）——
        # 也就是說它會在 live / degraded / missing / unwired **四種狀態下都出現**。
        # 故全段**不得**有任何「現在如何」的事實斷言（例如「K、D 值都抓得到」
        # 「把 K、D 數字當短線參考」）：那種句子只有在有值時才成立，缺資料那列
        # （`assess_stock(kd=None)` → 四態 missing 且沒有缺值原因可印，畫面上
        # 唯一的解釋就是這一段）與整檔抓取失敗那列（每盞燈都 missing）讀到的
        # 就是一句假話 —— 正是 §1 要擋的東西。
        # 現行寫法一律用**條件句／反事實句**（「就算…也…」）描述**結構事實**，
        # 因此四種狀態下都為真；「這輪到底有沒有抓到」交給它上面那行「狀態」
        # 與下面的缺值原因講，本段不重複、也不否認。
        no_level_reason=(
            "這盞燈**還沒有判定規則** —— 就算 K、D 都抓得到、鈍化／背離也照印，"
            "還是從來沒有人定義過「幾點算 🟢、幾點算 🔴」，所以它不會出等級。"
            "這**不是**「這類持股不適用」（KD 對個股完全適用），缺的是規則本身。"
            "這也與「這輪有沒有抓到」是**兩件不同的事**（那件事看上面那行「狀態」）："
            "抓到了照樣沒有等級，重跑一百次也一樣不會有等級。"
            "該怎麼看：有 K、D 數字時把它當短線參考自己判讀，不要等它變色；"
            "要不要續抱看上面的財報體檢與汰換建議。"
            "另外，因為它結構上不出等級，「幾盞給得出判定」那個分母不把它算進去 ——"
            "算進去會讓每一檔個股都永遠差一盞，那個數字就再也不會滿。"
        ),
    ),
    StationSpec(
...
#: 輸入齊、值也合法，但**這段期間完全沒有變異**（std≈0）→ 需要「離散程度」的
#: 統計量（布林位階 z 的分母）在數學上沒有定義。
#: 2026-08-27 新增。為什麼上面五個都不能用（逐一比對過，不是嫌它們不夠精準）：
#:   - `MISS_NO_INPUT` 說「可以重跑一次」—— **重跑一百次 std 還是 0**，是**錯的指引**；
#:     而且輸入其實抓到了（週K 一筆不缺），連前提都不成立。
#:   - `MISS_NOT_ENOUGH` 說「筆數不夠，等時間累積」—— 筆數是**夠的**（≥ 布林視窗）。
#:     等的是「它開始動」，不是「它變長」，兩者該做的事不同：一檔上市十年、
#:     這段期間平盤的 ETF，再等一年長度也不會是問題；而它可能永遠不會動（已停牌）。
#:   - `MISS_FETCH_FAILED` 會叫人去看該列的錯誤訊息 —— 沒有錯誤，其他燈都正常亮。
#:   - `MISS_NOT_APPLICABLE` 說「這類持股結構上不適用」—— ETF 結構上完全適用布林，
#:     只是**這一段期間**算不出來，下一段有波動就會恢復。
#:   - `MISS_CONTRACT_DRIFT` 會叫人去改程式 —— 值完全合法，程式沒 bug。
#: 掛載點（2026-08-27 起 **一處**）：`dividend_station_service._weekly_series_payload`
#: 的 `_line_miss` —— 週數夠、整條 z 卻無有限值，且週收本身零波動。
#: ⚠️ `dividend_station.light_235` 的「三軸全不可用 → `MISS_NO_INPUT`」**刻意沒改**：
#: 三軸同時掛掉時多半真的是 VIX + 週線都沒抓到，那裡的 `MISS_NO_INPUT` 是對的;
#: 要逐軸給原因得先讓 `Light235.miss_reason` 從單一字串變成 `dict[軸, 原因]`。
MISS_NO_VARIATION = "no_variation"
...
MISS_TEXT: dict[str, str] = {
    ...
    MISS_CONTRACT_DRIFT: "上游給的值不在約定的形態裡（不是沒資料，是資料長得不對）"
                         "—— 重跑不會好，這是程式要修的訊號，請回報。",
    MISS_NO_VARIATION: "這段期間價格完全沒有波動（標準差為 0）—— 這盞燈量的是"
                       "「離開平均多遠」，分母是 0 就沒有答案。"
                       "不是抓取失敗，重跑不會改變；多半是停牌或極低流動性，"
                       "等它開始有價格變動才算得出來。",
}
...
#:   3. 資料不足     → 結構性、等時間累積會好
#:   4. 整段零波動   → 資料齊、但這一段的統計量沒有定義（2026-08-27 插入）
#:   5. 該項沒抓到   → 單項的、重跑可能就好
#:   6. 不適用       → 根本不是問題，只有在**全部**都不適用時才該勝出
#:
#: 第 4 位為什麼在「資料不足」之後、「沒抓到」之前（沿用本表自己的「解釋力涵蓋
#: 範圍」標準）：歷史長度不足時**整條線都不存在**（涵蓋面大，該讓它贏）；
#: 而「整段沒動」解釋的是**單一統計量算不出來**，比「整檔某一項沒抓到」具體。
#: ⚠️ 這個順序**只影響「同一盞燈同時有多個原因時印哪一句」，不影響判燈**。
