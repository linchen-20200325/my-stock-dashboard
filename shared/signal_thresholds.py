...
# ════════════════════════════════════════════════════════════════
# 極端風險閘門（動作級警語:建議持股降至 EXTREME_TARGET_POSITION_PCT）
# ════════════════════════════════════════════════════════════════
#
# 為什麼是「兩腿共振」而不是單一門檻:
#   本 repo 既有的防禦訊號**全部太頻繁**,不能拿來當動作級閘門。2026-08-23 用
#   scripts/calibrate_macro_traffic.load_from_cache 逐日重建 19.4 年實測:
#       總經紅燈(任一防禦)  7.79 次/年,命中 47.6% 的月份
#       health < 35        18.12 次/年,命中 44.6% 的月份   ← 且此切點從未校準
#       融資過熱燈          恆紅(見 shared/macro_buckets.py 檔內自述「鑑別力歸零」)
#   一年響 8~18 次的東西寫上「持股降到 10%」,使用者會學會忽略它 —— 那比沒有更糟。
#
# 也不採「國際盤大跌」單腿:2026-08-23 用 mynews 的 71 天逐日歸檔實測,
# 現行 -1.5% 定義觸發 52% 的日子;就算一路收緊到任一領先市場 -5% 仍有 25%。
# 加深門檻的效率極差,而降到 ~8% 的定義召回率會掉到 25% 以下。
# （國際盤保留為**提示層**,見 shared/global_lead_markets.py —— 它 100% 不漏,
#   適合每天講一句「開盤留意」,不適合下動作指令。）
#
# 兩腿共振的實測(2026-08-23,data_cache/twii_ohlcv + finmind_inst,n=4,895 交易日 / 20.0 年):
#       20D <= -8%  單腿          272 日(5.56%)  48 episodes  2.40 次/年
#       外資 5 日 <= -500 億 單腿  451 日(9.21%) 104 episodes  5.20 次/年
#       兩腿同時成立              61 日(1.25%)   25 episodes  1.25 次/年   ← 採用
# 價格與資金**同時**壞掉才算,比單看價格穩:單腿各自的「常態波動」不會同時發生。
#
# ⚠️ 這個「兩腿共振」不是我們發明的 —— FOREIGN_5D_NET_THRESHOLD_YI 的 docstring
# 原文就寫著「配合大盤 20D 跌幅 -5% 才觸發紅旗」。此處只是把跌幅腿從 -5 收緊到 -8,
# 讓它從「紅旗」(2.40 次/年) 升格為「動作級」(1.25 次/年)。
#
# 漂移守衛:tests/test_extreme_risk_banner.py 會用同一份 parquet 重算頻率,
# 任一門檻被改鬆到超出 [0.8, 2.5] 次/年就紅燈。

EXTREME_TWII_20D_DROP_PCT: float = -8.0
"""極端風險閘門·腿一:加權指數 20 日跌幅（單位：%）。

比 TWII_20D_DROP_THRESHOLD_PCT(-5.0,「紅旗」級) 嚴格。**兩者刻意不合併** ——
-5 是「加速確認弱勢」的觀察門檻,-8 是「建議把持股降到一成」的動作門檻,
語意不同、消費者不同,合成一個常數會讓其中一邊被另一邊的調整波及。"""

EXTREME_TARGET_POSITION_PCT: int = 10
"""極端風險成立時建議的持股上限（單位：%）。

**不是新發明的數字**:shared/position_throttle.py 的防禦帶本來就是「持股 0~20%,
中值 10%」(health < 35 或 regime ∈ {bear, caution} 時的 exposure 上界)。
此處沿用同一個中值,只是換一個更罕見的觸發條件,避免全站對「防禦」有兩種說法。"""

EXTREME_STALE_MAX_CALENDAR_DAYS: int = 10
"""兩腿的來源 parquet 最多可以落後幾天（單位：**日曆日**）。

超過 → 該腿判為**無法評估**,整個閘門回 unknown 並在訊息中明講缺哪一腿。
§1:用過期資料算出來的 20 日報酬是錯的,而錯的數字比沒有數字危險。

**為什麼是日曆日而不是交易日**:本專案不使用第三方 trading calendar lib
(CLAUDE.md §4.5),parquet 之後有哪幾天是交易日無從得知 —— 寫「N 個交易日」
會是假精確。改用日曆日,語意誠實。

**為什麼是 10**:要能容忍農曆年連假(台股常休 7~9 個日曆日)而不誤報,
同時擋掉「cron 已經死了兩週」這種真故障。代價是連假期間資料真的變舊時
不會示警 —— 可接受,因為那段期間本來就沒有新交易可以應對。"""
...
# ════════════════════════════════════════════════════════════════
# F1 v19.184 — 今日作戰室「月虧損強制停機」(§3.3 反捏造)
#
# 這兩個數字原本 inline 在 `src/ui/tabs/macro/section_warroom.py`:
# 判定式寫 `_monthly_loss < -10`、同一張警示卡的文字寫「暫停操作 7 天」。
# 判定與文案各寫各的 → 改門檻只會動一邊(本批要消滅的 bug class)。
#
# ⚠️ 兩者皆為 **DESIGN**(行為紀律規則,非市場統計門檻),無外部權威來源;
#    §1 適用範圍是「資料輸出不得偽造」,UI 判讀門檻具名 + 文件化即可。
# ════════════════════════════════════════════════════════════════

MONTHLY_LOSS_HALT_PCT: float = -10.0
"""月虧損「強制停機」警示門檻(單位:%,負值)。
當月累積報酬 < -10% → 今日作戰室跳紅色警示卡,建議暫停操作。
原 section_warroom.py inline `_monthly_loss < -10`。"""

MONTHLY_LOSS_HALT_DAYS: int = 7
"""月虧損警示建議的暫停操作天數(單位:日曆天)。
純行為紀律建議,系統**不會**真的鎖住任何功能 —— 文案只寫「建議」。
原 section_warroom.py inline 文案「暫停操作 7 天」。"""


# ── F1 v19.184：大盤成交量急縮 / 急放警示(§3.3)──────────────────────
# 原 `src/ui/tabs/macro/section_chips.py` inline `_avg_vol * 0.7` / `* 1.5`,
# 而同一則警示的文案手算成「量縮超過 30%」「暴增 50% 以上」。
# 判定用**比值**、文案用**百分比**,兩者靠人腦換算 —— 改比值時文案必漏
# (§4.1 百分比 vs 小數陷阱的教科書案例)。現在文案由比值現算。
# ⚠️ 分母是「前 N-1 日均量」而非固定窗,N 由資料筆數決定(≥3 才判)。

MARKET_VOLUME_SHRINK_RATIO: float = 0.7
"""大盤成交量「急萎縮」警示:今日量 < 前 N-1 日均量 × 0.7(即量縮 >30%)。
原 section_chips.py inline `_avg_vol * 0.7`。DESIGN(UI 判讀線,無外部權威源)。"""

MARKET_VOLUME_SURGE_RATIO: float = 1.5
"""大盤成交量「急放」提示:今日量 > 前 N-1 日均量 × 1.5(即暴增 >50%)。
原 section_chips.py inline `_avg_vol * 1.5`。DESIGN(同上)。"""
