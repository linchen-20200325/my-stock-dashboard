...
#: 📌 **2026-09-26 有意識的規格變更，⛔ 不是漏刪；決策者：客戶。** §2 現為 **11 種** ——
#:    新增的 #11「▨ 無資料」**沿用本檔 `UI_STATE_META[UI_EMPTY]` 的圖示與文字**（HTML 徽章層直接讀本表，
#:    ⛔ 不抄字），但它的射程**只有**「上游成功算完、結果真的是 0／空」且**登記在
#:    `src/ui/views/page_today.py::V2_VALID_EMPTY_SPEC` 的卡**。
#:    ⇒ **下面這段判斷一字未變、仍然有效**：`MISS_NOT_ENOUGH`／`MISS_NO_VARIATION` **不是**有效的空結果
#:    （資料還不夠長／這段沒波動，是「還沒有值」而不是「算完了、就是空的」），
#:    ⛔ 不得據 #11 的新增把它們改畫 #11；本檔的 L0 十態**本次未動任何一個**。
