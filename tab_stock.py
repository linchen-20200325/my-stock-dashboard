...
【關鍵價位｜支撐壓力與停利停損】（系統實算，第四段戰術建議請直接引用，嚴禁自行臆測價位）
{_sr_str2}
...
【分析指令】
請依照以下步驟進行五維度交叉驗證：
步驟一：技術籌碼共振 - 確認技術面突破是否伴隨法人籌碼同步進場；並參考「近20日籌碼集中度」（正值=大戶吸貨）與「RS 相對強度」（是否強於大盤、具領漲資格）判斷主力動向。
步驟二：基本面背書 - 判斷股價波動是否有營收、毛利、EPS 增長支撐；納入「基本面先行指標（D2）」🟢🟡🔴 燈號，並檢視「龍頭擴產檢測」是否具備積極擴產的成長龍頭特徵。
...
- **參考進場區間**：（引用近20日支撐 {_lo20_p:.2f} 或朱家泓買點，結合 ATR={_atr2:.2f} 說明）
- **停利目標**：（目標1={_tp1_p}、目標2={_tp2_p}，並對照近20日壓力 {_hi20_p:.2f}）
- **停損點位**：（建議停損={_sl_p}；絕對停損參考 ATR 1.5 倍 {round(price2 - _atr2 * 1.5, 2):.2f}；盈虧比={_rr_p}x）
