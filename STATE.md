...
- **目前版本**：v18.167_BacktestOptimizationTab（新增 top-level Tab「🧪 回測找參數」排在「💰 ETF質借模擬」之後 — 完整版多因子權重最佳化 workflow：📖 操作說明 expander（含核心數學公式 + 6 slider 解釋）+ 1️⃣ TWII 回撤門檻 + 抓資料按鈕 → 2️⃣ 因子 multiselect + Plateau 目標 radio (F1/Sharpe) + 6 顆 slider（grid 步長 / 鄰域半徑 / λ / train / test / 警戒線）+ 3️⃣ 高原最佳權重 metric 卡 + CSV 匯出 + 4️⃣ plateau 2D heatmap / 3D surface toggle + 5️⃣ walk-forward 各折表 + OOS F1/Sharpe + CSV；引擎複用 v18.165 multi_factor_optimization.py 零重寫；macro validation 內 expander 並存「快速版」）
