...
| **引擎** | `scoring_engine.py` · `scoring_helpers.py`（PR #61 抽 3 純函式：fundamental_score / health_score / health_grade — Phase 7H 補 50 unit test）· `financial_health_engine.py` · `market_strategy.py` · `risk_control.py` · `backtest_engine.py` · `unified_decision.py` · `v4_strategy_engine.py` · `v5_modules.py` · `yield_screener.py` · `flow_engine.py`（全球資金流向純計算：區域 ETF 相對強弱排名 + 252d 滾動 Z-score risk-on/off 合成，零 streamlit、13 unit test）|
| **技術指標** | `tech_indicators.py`（PR #58 從 app.py 抽出 6 個純函式：RSI/IBS/VR/KD/BB/VCP，零 Streamlit 依賴 — Phase 7H 補 47 unit test）|
| **出場訊號** | `exit_signals.py`（三維出場綜合判斷：利空新聞 LLM 情緒 / 技術轉空 / 籌碼倒貨 → 3/2/1 計分分級；純邏輯零 UI、Gemini 由呼叫端注入、新聞判讀 `st.cache_data` 6h、16 unit test。個股 + 個股組合 tab 共用）|
...
| (本輪) | feat(hot-money): 熱錢監測 — 三角交叉（外資 × 匯率 × 背離偵測）。新模組 `hot_money.py` 含 `build_signals` 純函式 + `_twd_df_to_series` + `fetch_foreign_flow_series`（複用 `leading_indicators.finmind_get`，30min cache）+ `render_hot_money_section`（inline expander UI，象限圖 + 時序 + 背離清單）。`tab_macro.py` pivot_signals 後加 expander「💵 熱錢深度監測」，深化既有第 4 條「台幣升貶」單變量訊號為雙變量交叉。FINMIND_TOKEN 沿用既有 secrets，TWD=X 從 `_tw2.get('新台幣匯率')` 複用，零重複 API 呼叫。16 個單元測試（9 個 state 分類 + 7 個 yfinance df 解析） | (待 push) |
...
| (本輪) | perf: 股票儀表板下載/抓取提速 4 處 — (1) `tab_stock_picker` 三階段篩選序列迴圈改 `ThreadPoolExecutor(max_workers=5)` 並行（`_check_one_stock` 全程獨立 requests+yfinance、零 st.*、無共享 loader → 線程安全無鎖；index→future 映射保留輸入順序 + `_blank_pick_result` fallback）首跑 ~3-5x；(2) `tab_stock_grp:288` 移除 `get_combined_data(,300,)` 重抓改用 :143 已抓的 `df4`（批次每檔 API 2→1，已驗證 score_single_stock 不依賴 DatetimeIndex）；(3) `fetch_financial_statements`(data_loader) + (4) `fetch_revenue`(app) 補 `@st.cache_data(ttl=1h)`。釐清：fetch_price/dividend/financials/quarterly/get_combined_data 早有快取非瓶頸。驗證：ruff 0 新錯 + 320 評分測試綠 + 並行順序/fallback 功能測試綠 | (待 push) |
| (本輪) | perf: `fetch_financial_statements`(data_loader) 內 3 個獨立 FinMind 呼叫（資產負債表/現金流/損益表）改 `ThreadPoolExecutor(3).map` 並行，首次載入由序列 3 趟 → ~1 趟（~3x）；map 保序故解包對位不變、邏輯零變動。供單股財報區 / MJ 體檢 / 選股器共用。**第二輪探索結論**：上輪審計「未快取」候選幾乎全是假警報——總經 4 fetcher + market_strategy 2 個在線程池內跑且結果存 session_state（加快取既危險又低效益）；`get_quarterly_data`/`get_quarterly_bs_cf` 已被 cached 的 `fetch_quarterly`/`_extra` 包住；`fetch_fund_nav` 無呼叫點。簡單快取紅利已耗盡，剩「並行化獨立網路呼叫」。驗證：ruff 0 新錯 + 17 financial_health 測試綠 + map 保序/端到端 mock 測試綠 | (待 push) |
| (本輪) | perf(生產資料/rerun 重算): `tab_stock` PB 股價淨值比河流圖的 `yf.Ticker().info`（bookValue）抓取擷取成模組級 `@st.cache_data(ttl=1天) _fetch_bps(sid)`（比照既有 `_fetch_share_capital`）。原本在 line 192 `else:` 無條件渲染區內、**每次 Streamlit rerun 都阻塞 ~1-3 秒**（單股 tab 卡頓主因）→ 改每股每日一次。**第三輪 rerun-重算稽核結論**：ETF 全部（蒙地卡羅/重疊矩陣/葡萄串/同儕排名）+ 批次(t3_data)+ 總經(do_refresh) 重運算皆被 button/session_state/快取守衛，無問題；唯一破口是單股 tab 渲染區（抓取被 `if t2_run` 守衛、但渲染每 rerun 全跑），其中 PB 的內嵌 yfinance 是真兇，殖利率/PE 河流圖 rolling/merge_asof 雖每 rerun 重算但僅數十 ms。驗證：ruff All passed + 快取命中行為測試（2330 呼叫 2 次網路僅 1 次）| (待 push) |
| (本輪) | perf(總經抓取): 延長既有 pickle 快取 TTL 10→30 分 — `data_config.TTL_CONFIG` institutional/margin_balance 600→1800、`daily_checklist.fetch_single` 內聯 /tmp 快取 `<10` 分→`<30` 分。讓總經的法人/融資/指數(DJI/SOX/DXY/^TWII/TWD/美股)在「重訪/新 session/重按更新」30 分內秒開。**第四輪總經稽核重大更正**：(1) 總經抓取**早已全平行化**(每 job 一 worker，非序列問題)且 session_state 守衛；(2) 多數 fetcher **早有 ~10 分 pickle 跨 session 快取**(fetch_single/institutional/margin/adl + proxy_helper.fetch_url 300s)——我第二輪「線程內 @st.cache_data 失敗→沒快取」的判斷**被實證推翻**(實測 ThreadPoolExecutor 內 @st.cache_data 完全正常)，且 daily_checklist:532 註解顯示開發者早改用 pickle 快取。(3) 真正全裸的 `build_leading_fast`(4 序列 FinMind)本欲加 pickle 快取，但測出「FinMind 暫時失敗仍組出全 None 骨架、`filled` 仍>0」→ 會把暫時性故障黏 30 分，依 §5 卡關協議放棄(需更可靠的資料有效性判斷，列後續)。fetch_single 確認只餵總經指數/匯率/美股(非個股即時現價，不回歸 #94)。驗證：707 測試綠(排除 bs4 缺套件 3 檔) + TTL 值斷言 | (待 push) |
| (本輪) | perf(總經抓取-續): 補完上輪放棄的 `build_leading_fast`(先行指標，4 序列 FinMind)快取 — 改用**可靠 guard `_fm_ok`**(TX/MTX/TXO/inst 主來源任一有回資料才寫快取)取代上輪失準的 `filled`。沿用 fetch_single 內聯 /tmp pickle 模式(回新物件、線程安全)，30 分 TTL。**關鍵**：FinMind 暫時失敗(全空)時雖仍組出 14 列 None 骨架，但 `_fm_ok=False` → **不快取**，保留下次刷新重試(不黏住暫時性故障)。冷抓也能省一趟。驗證：ruff 166(0 新增) + 雙情境快取行為測試(有資料→寫快取且第二次命中；全空→不快取) | (待 push) |
| (本輪) | feat(總經): 新增「🌊 全球資金流向」一節(第一節國際市場後)。新模組 `flow_engine.py`(純計算)：區域 ETF(SPY/VGK/EWJ/FXI/EEM/EWT)近 5/20 日報酬排名當資金流入流出代理 + 跨資產 risk-on/off 分數(SPY/TLT、HYG/LQD、MOVE/VIX、VIX、UUP、GLD、USDJPY 的 252d 滾動 Z-score×方向取平均→-100..100)。`daily_checklist.fetch_flow_snapshot()` 並行抓取+去重+/tmp pickle 快取 30 分(核心 SPY 抓到才寫)。**誠實標記**：真實基金流量(EPFR)為付費資料,本區用相對強弱代理;XCCY 基差、穩定幣 SSR 因免費源不可得未納入(UI 明示)。隔離式設計**完全不動**脆弱的 job 編排。驗證：13 flow_engine unit test + 端到端煙霧(去重/快取命中/render 資料前處理無崩潰/risk 方向正確) + 720 測試全綠 | (待 push) |
| (本輪) | feat(出場訊號): 三維出場點綜合提示。新模組 `exit_signals.py`(純邏輯)：①利空新聞(Gemini 情緒判讀 利空/中性/利多+信心，沿用 `gemini_call` 金鑰池、`st.cache_data` TTL 6h)②技術轉空(空頭排列/跌破季年線/KD高檔死叉/週MACD翻負/高乖離，含強訊號或≥2條即成立)③籌碼倒貨(`analyze_20d_chips_from_df` 回🔴大戶倒貨)。命中維度數計分 3🔴/2🟠/1🟡/0🟢。**個股 tab**：進出場訊號上方加綜合 banner(舊各策略訊號保留)，新聞自動判讀。**個股組合 tab**：④汰弱留強表新增「出場」欄(技術+籌碼兩維即時)，利空新聞 LLM 第三維由「🤖 AI 掃利空」鈕按需觸發(省 Gemini 額度)。驗證：16 exit_signals unit test + ruff All passed + 736 測試全綠 | (待 push) |
| #98 | perf(總經): 解決「一鍵更新又慢又只載入片段」。①先行指標 `build_leading_fast` 4 個獨立 FinMind 批次 API（TX/MTX/TXO 期權法人 + 三大法人現貨）由序列（各 25s×2，最壞 ~150-200s 全卡同步關鍵路徑）改 `ThreadPoolExecutor(4)`，降為 ~單一最慢源；刪死碼 `_job_li` + 失效 import。②`macro_core.fetch_tw_pmi` 9 段來源序列 fallback（最壞 ~100s+、超過 macro 池 70s timeout → PMI 變片段且無靜態備援）抽成 9 個模組級 `_pmi_src_*`（解析 body 零變動）並行賽跑、**依原優先序取值**（語義不變）→ ~單一最慢源（~15s）穩落預算內。**重大更正**：(1) 原規劃替 macro/M1B/乖離率加跨 session 快取，讀碼發現「一鍵更新」on_click 會先清光所有快取再抓 → 對該慢路徑零幫助，撤掉改做並行化；(2) 外銷 export 已最佳化（MOF 月迴圈 2 月早停 + 靜態備援永遠有值），不需改。驗證：py_compile + ruff 無新增 + 783 測試綠 + 先行指標 4 呼叫真並行煙霧測試 + PMI 4 場景 mock 調度測試（優先序/遞補/慢源不擋/全失敗回退，8×0.4s 並行實測 0.40s） | cc12f30 |
| #100 | fix(proxy): `fetch_url` 加 NAS 中繼站 fallback。元兇＝官方源（NDC 景氣指標/CIER PMI）對資料中心 IP 回 **403**，9 條 PMI 來源全被擋（台灣 PMI 數據其實存在：2026-04=60.3%、03=55.4%）。在 `return None` 前加一層：Squid proxy+直連皆失敗時，把 url+params 組成完整網址交給既有 `nas_relay_fetch()`（家用 NAS `/proxy?url=`，台灣住宅 IP 代抓）→ 9 條 PMI 及其他被 403 擋的官方源全自動受惠。**僅失敗路徑觸發**、params 正確併入查詢字串、未設 `NAS_BASE_URL` 時回 None 零 regression。部署前提：Secrets 需 `NAS_BASE_URL`(或 `NAS_RELAY_URL`)+`NAS_API_KEY`。驗證：py_compile + ruff 無新增 + 783 測試綠 + relay wiring mock 雙場景（命中含 params 併入/未設定回 None） | PR #100 |
| #102 | fix(tab_macro): 修掉作戰室整頁炸的 `StreamlitAPIException: Expanders may not be nested inside other expanders`。PR #101 把「💵 熱錢深度監測 — 三角交叉」`st.expander` 放進「📊 拐點詳細分析」expander 內部 → Streamlit 禁止巢狀 → render_tab_macro 全頁掛掉。把 `if _twd_df` 由 indent 12 退到 indent 8，讓內層 expander 成為外層 expander 的 sibling（仍在同一個 `if _mkt_info:` 區塊、`_twd_df` 在 scope、UI 順序不變）。驗證：ast.parse + ruff All passed + `tests/test_hot_money.py` 16 passed + grep 確認兩個 expander 同層 | PR #102 |
| #104 | perf(tab_macro): 進頁完全不抓資料 — 5 處非 `do_refresh` 的 fetch（`fetch_flow_snapshot` 全球資金流向、`_fetch_otc_via_finmind` OTC、`fetch_adl` 60 天補救、`fetch_twse_breadth` 即時漲跌、`fetch_macro_snapshot` VIX/CPI/10Y/DXY/PCR）全部加 `if _load_heavy:` 守衛。元兇＝NAS Proxy 慢時這些 fetch 在背景跑但畫面已先 render，用戶誤以為「全部 🔴 未取得」。改完後：未按按鈕 → render only 零外連 + 統一「📡 點擊一鍵更新...」placeholder；按按鈕 → `chips_loaded=True` 永久 latch，往後 rerun 不卡。**不動任何抓資料邏輯本身**（純加 gating）。驗證：ast.parse + ruff All passed + 799 測試全綠 | PR #104 |
| #106 | perf(tab_macro): 未按按鈕時隱藏 stat_card 行（消除「載入中...」殘像）。#104 後 5 處 fetch 已守衛，但 stat_card row（國際 5、台股 2、科技 8 共 15 張）即使 `has_data=False` 仍 render 灰底「載入中...」骨架卡片，用戶誤以為背景在跑 fetch。3 處 row render 前加 `if _load_heavy:` 守衛（line 2139/2277/3122），未按按鈕整列不顯示（策略 3 banner 已給「請點擊一鍵更新」訊息，不需重複 placeholder）。純 UI gating，不動任何資料路徑。驗證：ast.parse + ruff + 799 測試全綠 | PR #106 |
| #108 | perf(tab_macro): 未按按鈕前 early return — 整個總經 tab 只剩按鈕。#104/#106 已守衛 fetch + stat_card，但 section header + 策略 banner + 「點擊載入XX」placeholder 還是滿版「灰底色塊噪音」。用戶要求「完全空白只剩 🚀 按鈕」→ 在 `_load_heavy` 計算後（line 541）加 `if not _load_heavy: return`，跳過全部下方 section render。保留：上方 FinMind Token banner + 🚀 按鈕 + 時間戳；隱藏：拐點 / 國際 / 全球資金流向 / 台股大盤 / 籌碼 / 策略手冊 / AI 總裁決所有 section。按按鈕後 `do_refresh=True → _load_heavy=True` 走原本全流程；按過一次後 `chips_loaded=True` 永久 latch | PR #108 |
| #110 | fix(tab_macro): 三大法人 FinMind 補救 parsing bug — 用戶報「跑不出資料」，啟動全域除錯協議。Phase 1 抓 debug log 發現 `[FinMind-Inst] status=200 rows=30` 但 `✅ {}`（30 筆有資料但 parsing 完空 dict）。Phase 3 不靠 log 從 codebase 自驗：`tw_macro.py:151` + `hot_money.py:157` 都用英文 key 匹配（`Foreign_Investor` / `Investment_Trust` / `Dealer_*`），但 `tab_macro.py:680` 用中文 `'外資' in _nm` 永遠 miss。改成雙語匹配（英文 primary + 中文 fallback），同時聚合 `Foreign_Investor`+`Foreign_Dealer_Self`→外資、`Dealer_Hedging`+`Dealer_self`→自營商。驗證：mock 30 rows 英文 path 算出 (外資 21, 投信 5, 自營 4) 億 + 中文 fallback 也綠 + ruff + 799 測試全綠。Phase 4 自動清理：debug print 已刪除、`.bak` 移除、`*.bak`/`debug.log` 加進 .gitignore | PR #110 |
| #111 | fix(health_inspector): 三大法人空 dict 不再被誤判為新鮮。用戶報「外資現貨 --」但異常清單沒列出。根因：line 379 用 `_inst_df is not None and not isinstance(DataFrame)` 判定 → 但 `{}` 同時滿足兩條件 → 被誤標 🟢 → 異常清單漏抓。新增 `_inst_has_data` 真值檢查（dict 需 len>0 或 DataFrame 非 empty），date_str 只在真有資料時才給。與 #110 一起，保證：真實有資料→卡片有值+清單不出現；真實空白→卡片 --+清單列 🔴。驗證：ast.parse + ruff 我這次 0 新 error（3 個 E701 為 pre-existing）+ 799 測試全綠 | PR #111 |
