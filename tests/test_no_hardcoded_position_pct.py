...
_KNOWN_DEBT: dict[str, tuple[str, ...]] = {
    ...
    # ✅ 2026-09-21 S1-6 合規改寫已結案（WH1）：三個分支的 `msg` 已改為
    #    「命中哪條規則 + 對照參數見 stock_pct/bond_pct/cash_pct」的陳述句，
    #    句中不再有任何硬編碼持股百分比 → 債務條目移除（註解留追溯，非漏刪）。
    #    ⚠️ `stock_pct/bond_pct/cash_pct` 三個**數值欄位**仍是硬編碼（屬回傳
    #    結構，非文案，不在合規改寫射程）→ 「接線就多一套持股建議」的風險**未解**，
    #    仍應如原註所述：下一版直接刪或改吃 SSOT。
}
...
_FALSE_POSITIVE_RE: tuple[tuple[re.Pattern, str], ...] = (
    (re.compile(r'現金\s*[^\n]{0,4}總資產'),
     '財報體質「現金佔總資產 >25%」(「氣長不長」框架) —— 是選股門檻，不是投組現金部位'),
