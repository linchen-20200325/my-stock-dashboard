"""v18.185 fin_health_diff 純函式單元測試（不打網路/LLM）。

v19.174 去識別化：模組自 `mj_health_diff` 改名為 `fin_health_diff`，
主函式 `diff_mj_health` → `diff_fin_health`（皆經 package lazy forward 取用）。
"""
...
class TestScore:
    ...
    def test_prefix_match(self):
        # 體檢 engine 偶爾吐 "Exception_Pass (條件A：現金充足)"
...
# _walk_statuses
# ════════════════════════════════════════════════════════════════
def _fh_mini(cash="Pass", dso="Acceptable", gm="Good", om="Moderate",
...
# ════════════════════════════════════════════════════════════════
# diff_fin_health — 核心比對
# ════════════════════════════════════════════════════════════════
class TestDiffMjHealth:
    def test_all_improved(self):
        prev = _fh_mini(cash="Fail", dso="Fail", gm="Hard Work")
        curr = _fh_mini(cash="Pass", dso="Acceptable", gm="Good")
        v = diff_fin_health(prev, curr, stock_id="2330")
    ...
    def test_all_deteriorated(self):
        prev = _fh_mini(cash="Pass", dso="Acceptable", gm="Good", om="Excellent")
        curr = _fh_mini(cash="Fail", dso="Fail", gm="Hard Work", om="Fail")
        v = diff_fin_health(prev, curr, stock_id="2330")
    ...
    def test_mixed_signals(self):
        # 兩個變好、兩個變差，平手 → mixed
        prev = _fh_mini(cash="Fail", dso="Pass", gm="Hard Work", om="Excellent")
        curr = _fh_mini(cash="Pass", dso="Fail", gm="Good", om="Fail")
        v = diff_fin_health(prev, curr, min_net_delta=1)
        # 2 改善 + 2 惡化, net = 0 → mixed
        assert v.verdict == "mixed"

    def test_min_net_delta_buffer(self):
        # 只有 1 項變好 → 預設 min=1 → improving
        prev = _fh_mini(cash="Fail")
        curr = _fh_mini(cash="Pass")
        v = diff_fin_health(prev, curr, min_net_delta=1)
        assert v.verdict == "improving"
        # 提高門檻 min=2 → 應降為 mixed
        v2 = diff_fin_health(prev, curr, min_net_delta=2)
        assert v2.verdict == "mixed"

    def test_is_turnaround_flag(self):
        # Core_Business_Profitable No → Yes（本業由賠轉賺）
        prev = _fh_mini(cbp="No")
        curr = _fh_mini(cbp="Yes")
        v = diff_fin_health(prev, curr)
        assert v.is_turnaround is True
        assert v.is_breakdown is False

    def test_is_breakdown_flag(self):
        prev = _fh_mini(cbp="Yes")
        curr = _fh_mini(cbp="No")
        v = diff_fin_health(prev, curr)
    ...
    def test_missing_prev_returns_empty_verdict(self):
        v = diff_fin_health(None, _fh_mini(), stock_id="X")
        assert v.improve_count == 0
        assert v.deteriorate_count == 0
        assert v.verdict == "stable"

    def test_missing_curr_returns_empty_verdict(self):
        v = diff_fin_health(_fh_mini(), None)
        assert v.verdict == "stable"
...
# ════════════════════════════════════════════════════════════════
def _snap(sid, prev_fh, curr_fh):
    return {"id": sid, "prev": prev_fh, "curr": curr_fh}


class TestScreenHealthChanges:
    def test_mode_improving_only(self):
        snaps = [
            _snap("UP", _fh_mini(cash="Fail"), _fh_mini(cash="Pass")),
            _snap("DOWN", _fh_mini(cash="Pass"), _fh_mini(cash="Fail")),
            _snap("STABLE", _fh_mini(), _fh_mini()),
        ]
    ...
    def test_invalid_mode_defaults_to_both(self):
        snaps = [_snap("UP", _fh_mini(cash="Fail"), _fh_mini(cash="Pass"))]
    ...
    def test_min_net_delta_propagated(self):
        snaps = [_snap("X", _fh_mini(cash="Fail"), _fh_mini(cash="Pass"))]
...
class TestHelpers:
    ...
    def test_verdict_to_dict_full_shape(self):
        v = diff_fin_health(_fh_mini(cash="Fail"), _fh_mini(cash="Pass"))
        d = v.to_dict()
