...
def _clear_caches():
    ...
    import tab_stock
    for _fn in ('_fetch_pbratio_from_twse', '_fetch_industry_category',
                 '_fetch_bps_from_finmind', '_fetch_bps'):
        try:
            getattr(tab_stock, _fn).clear()
        except Exception:
            pass
    yield
    for _fn in ('_fetch_pbratio_from_twse', '_fetch_industry_category',
...
class TestTwsePbratioFetch:
    ...
    def test_empty_df_returns_zero(self):
        import tab_stock
        with patch('yield_screener.fetch_twse_yield_pe',
                    return_value=pd.DataFrame()):
            pb = tab_stock._fetch_pbratio_from_twse('2330')
        assert pb == 0.0

    def test_sanity_extremely_high_returns_zero(self):
        """PB > 100 視為異常 → 0.0。"""
        import tab_stock
        df = _mk_twse_df([{'代碼': '2330', '名稱': '台積電', '股價淨值比': 500.0}])
        with patch('yield_screener.fetch_twse_yield_pe', return_value=df):
            pb = tab_stock._fetch_pbratio_from_twse('2330')
        assert pb == 0.0

    def test_sanity_zero_or_negative_returns_zero(self):
        """PB <= 0 視為異常 → 0.0。"""
        import tab_stock
        df = _mk_twse_df([{'代碼': '2330', '名稱': '台積電', '股價淨值比': 0.0}])
        with patch('yield_screener.fetch_twse_yield_pe', return_value=df):
            pb = tab_stock._fetch_pbratio_from_twse('2330')
        assert pb == 0.0
...
class TestIndustryBands:
    def test_financial_bank(self):
        import tab_stock
        assert tab_stock._get_pb_bands('銀行業') == (0.5, 0.9, 1.2)

    def test_financial_insurance(self):
        import tab_stock
        assert tab_stock._get_pb_bands('金融保險業') == (0.5, 0.9, 1.2)

    def test_growth_semiconductor(self):
        import tab_stock
        assert tab_stock._get_pb_bands('半導體業') == (1.5, 2.5, 4.0)

    def test_growth_electronics(self):
        import tab_stock
        assert tab_stock._get_pb_bands('電子工業') == (1.5, 2.5, 4.0)

    def test_growth_optoelectronics(self):
        import tab_stock
        assert tab_stock._get_pb_bands('光電業') == (1.5, 2.5, 4.0)

    def test_mfg_default(self):
        import tab_stock
        assert tab_stock._get_pb_bands('鋼鐵工業') == (0.8, 1.5, 2.5)

    def test_empty_industry_uses_mfg_default(self):
        import tab_stock
        assert tab_stock._get_pb_bands('') == (0.8, 1.5, 2.5)
        assert tab_stock._get_pb_bands(None) == (0.8, 1.5, 2.5)

    def test_unknown_industry_uses_mfg_default(self):
        import tab_stock
        assert tab_stock._get_pb_bands('外星科技業') == (0.8, 1.5, 2.5)
...
class TestIndustryLabel:
    def test_financial_label(self):
        import tab_stock
        assert '金融' in tab_stock._pb_bands_label('銀行業')

    def test_growth_label(self):
        import tab_stock
        assert '成長科技' in tab_stock._pb_bands_label('半導體業')

    def test_mfg_label(self):
        import tab_stock
        assert '製造業' in tab_stock._pb_bands_label('鋼鐵工業')

    def test_empty_falls_back_to_mfg_default(self):
        import tab_stock
        assert tab_stock._pb_bands_label('') == '製造業預設'
...
class TestFetchIndustryCategory:
    def test_extracts_industry_from_finmind(self):
        ...
        with patch('requests.get', return_value=_mk_finmind_resp(rows)):
            ind = tab_stock._fetch_industry_category('IC_TST_8001')
        assert ind == '半導體業'

    def test_empty_data_returns_empty_string(self):
        import tab_stock
        with patch('requests.get', return_value=_mk_finmind_resp([])):
            ind = tab_stock._fetch_industry_category('IC_TST_8002')
        assert ind == ''

    def test_no_industry_field_returns_empty(self):
        import tab_stock
        rows = [{'stock_id': '2330', 'stock_name': '台積電'}]  # 無 industry_category
        with patch('requests.get', return_value=_mk_finmind_resp(rows)):
            ind = tab_stock._fetch_industry_category('IC_TST_8003')
        assert ind == ''
    ...
    def test_http_500_returns_empty(self):
        ...
        with patch('requests.get', return_value=bad_resp):
            ind = tab_stock._fetch_industry_category('IC_TST_8004')
        assert ind == ''
...
class TestTwseBpsBackCalc:
    def test_back_calc_bps_from_pbratio_and_price(self):
        ...
        with patch('yield_screener.fetch_twse_yield_pe', return_value=df):
            pb = tab_stock._fetch_pbratio_from_twse('2330')
        # call site 的反推邏輯：bps = price / pb
        bps = 750.0 / pb if pb > 0 else 0
        assert 149.0 < bps < 151.0  # = 150
