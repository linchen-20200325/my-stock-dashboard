"""tests/test_caisen_ui_mounted.py — v19.162 守衛:老師目標價 Tab。
...
def test_render_caisen_for_ticker_importable():
    """v19.163:老師核心可重用元件可直接 import(不再有獨立分頁,見 test_reusable_*)。"""
...
def test_no_standalone_caisen_tab():
    """v19.163 user 要求:老師不設獨立分頁(只內嵌個股/組合)。"""
    src = (_REPO / "app.py").read_text(encoding="utf-8")
    assert "render_caisen_targets_tab" not in src, "殘留獨立老師分頁掛載"
