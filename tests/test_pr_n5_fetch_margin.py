...
class TestFetchMarginBalanceExtracted(unittest.TestCase):
    ...
    def test_reexport_identity(self):
        from src.services import fetch_margin_balance as _m1
        from src.data.daily import fetch_margin_balance as _m2
        self.assertIs(_m1, _m2)

    def test_daily_checklist_no_inline_def(self):
        """src/services/daily_checklist.py 應已無原 def fetch_margin_balance(僅 re-export)。"""
        import ast
        with open('src/services/daily_checklist.py', encoding='utf-8') as f:
    ...
    def test_caller_health_inspector_works(self):
        """health_inspector L204 `from src.services.daily_checklist import fetch_margin_balance` 仍 OK。"""
        # 不執行(無 token),只驗 import 路徑
        import importlib
        _mod = importlib.import_module('src.services.daily_checklist')
