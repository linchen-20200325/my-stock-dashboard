...
def _src(p="src/ui/tabs/tab_macro.py"):
    # P3-D4 v18.389:_job_* 全下沉至 src/services/macro_fetch_orchestrator.py;
    # 守衛斷言改掃 tab_macro + orchestrator 合集。
    base = open(p, encoding="utf-8").read()
    try:
        base += open("src/services/macro_fetch_orchestrator.py", encoding="utf-8").read()
...
class TestLeadingInPool:
    def test_li_job_registered_in_pool(self):
        ...
        # P3-D4:_jobs dict 對齊用空白縮排不同,改檢 'li': _job_li
        assert "'li': _job_li" in src or "'li':           _job_li" in src, \
            "li 未併入 _jobs 平行池"
    ...
    def test_serial_leading_block_removed(self):
        ...
        # 結果改從 pool 收回(P3-D4 後在 orchestrator 內部用 _results;
        # tab_macro 改從 bundle['df_li_a'] 取)
        assert ("df_li_a = _results.get('li')" in src
                or "df_li_a    = _bundle['df_li_a']" in src), \
            "li 結果未從 pool/bundle 收回"
